import React__default, { useMemo, useState, useRef, useEffect, useContext, useCallback, useLayoutEffect } from 'react';
import { A11yMeaningfulLabelContext } from '../../components/A11yMeaningfulLabel/shared/A11yMeaningfulLabelContext.js';
import { hostClass } from '../../components/Grid/shared/Grid.styles.js';
import { handleKeydownListFlyout } from '../../components/_SelectFlyout/HandleKeyDownListFlyout.js';
import { utilityClassScroll, utilityClassScrollPadding16, utilityClassScrollPadding22 } from '../../styles/utilityClassesScroll.styles.js';
import { animationDuration } from '../../styles/variables.js';
import { breakPoints, getViewportMediaQueryRange } from '../../styles/viewport.js';
import { uniqueId, isTargetInsideContainer } from '../DomUtils.js';
import { Key } from '../Keys.js';
import { setLsgTimeout, clearLsgTimeout } from '../timing.js';
import { TransitionHandler } from '../transition.js';
import { getInitialViewportSize, addViewportCallback, removeViewportCallback, getViewportSize } from '../windowEvents/ResizeEvents.js';
import { isPreferReducedMotion } from './animation.js';
...
function useUniqueId(prefix, defaultId) {
    try {
        ...
        return useMemo(() => defaultId || uniqueId(prefix), [prefix, defaultId]);
    }
    catch (error) {
...
function useMenu(menuItems) {
    const [open, setOpenButton] = useState(false);
    const [focussedIndex, setFocussedIndex] = useState(undefined);
    const [scrollFocussedElementIntoView, setScrollFocussedElementIntoView] = useState(false);
    const [buttonRef, containerRef, focussedRef, flyoutRef] = [
        useRef(null),
        useRef(null),
        useRef(null),
        useRef(null),
    ...
    const onKeyDown = event => handleKeydownListFlyout({
        event,
        open,
        focussedIndex,
        options: menuItems.map(({ children: label }) => ({ label: label })),
        onChange: () => {
        ...
        additionalConfirmKeys: [Key.Space],
        useTypeAhead: true,
    ...
    useEffect(() => {
        ...
        setLsgTimeout(() => focussedRef?.current?.focus());
    }, [focussedIndex, open]);
    return [
        { open, focussedIndex },
        { setOpen: setOpenButton, setFocussedIndex },
        { buttonRef, flyoutRef, focussedRef, containerRef },
        { handleMouseOver, onKeyDown },
    ];
}
...
function useDwindle(actionProps) {
    ...
    const minClickTimeExceeded = useRef(true);
    const minPressTimeExceeded = useRef(true);
    const mouseupPending = useRef(false);
    const keyboardupPending = useRef(false);
    let [hasKeyboardFocus, setHasKeyboardFocus] = useState(false);
    let [hasMouseHover, setHasMouseHover] = useState(false);
    ...
    if (actionProps.a11yMeaningfulLabel) {
        ...
        } = useContext(A11yMeaningfulLabelContext));
        ...
        ({ hasMouseHover, isActive: clicked } = useContext(A11yMeaningfulLabelContext));
        setClicked = () => { };
        setHasMouseHover = () => { };
        setHasKeyboardFocus = () => { };
        // Keyboard focus is on the Card/Teaser and should not be set on the Button
    ...
    useEffect(() => () => clearLsgTimeout(timeout.current), []);
    useEffect(() => {
    ...
    function onMouseDown(ev) {
        ...
        if (!actionProps.disabled && !clicked && !actionProps.nonInteractive) {
            ...
            if (timeout.current) {
                clearLsgTimeout(timeout.current);
            }
            timeout.current = setLsgTimeout(() => {
...
function useLinkHover(actionProps) {
    ...
    if (actionProps.a11yMeaningfulLabel) {
        ...
        } = useContext(A11yMeaningfulLabelContext)); // TODO: Refactor. Hook is called conditionally
        // Keyboard focus is on the Card/Teaser and should not be set on the Link
        hasKeyboardFocus = false;
    }
    else if (actionProps.nonInteractive) {
        // TODO: Refactor. Hook is called conditionally
        ({ hasKeyboardFocus, hasMouseHover } = useContext(A11yMeaningfulLabelContext));
        setHasMouseHover = () => { };
        setHasKeyboardFocus = () => { };
        // Keyboard focus is on the Card/Teaser and should not be set on the Link
        hasKeyboardFocus = false;
...
const viewportsArray = Object.keys(breakPoints).reverse();
...
function useViewportRange(min, max) {
    const [matches, setMatches] = useState(isInViewportRange(getInitialViewportSize(), min, max));
    useEffect(() => {
        const mql = window.matchMedia(getViewportMediaQueryRange(min, max));
...
function useViewport() {
    ...
    useEffect(() => {
        addViewportCallback(setSize);
        return () => {
            removeViewportCallback(setSize);
        };
    }, []);
    return size;
}
function useResize(onResize, deps = []) {
    ...
    const onResizeCallback = useCallback(onResize, deps);
    useEffect(() => {
...
function useViewportListener(callback, doInitialCall = true) {
    ...
    const viewportSize = useRef(getInitialViewportSize());
    const onResize = () => {
...
function useTransitionState(isVisible, duration) {
    const transitionHandler = useMemo(() => new TransitionHandler(), []);
    const [transitionState, setTransitionState] = useState(transitionHandler.makeTransitionState("unmounted"));
    ...
    return { transitionState, transitionHandler };
}
...
function useTransition2(enter, options, onTransition = onTransitionDefault) {
    const animationsActivated = !isPreferReducedMotion();
    const [currentlyAnimating, setCurrentlyAnimating] = useState(animationsActivated);
    const activeSequence = currentlyAnimating ? options.sequence : options.reducedAnimationSequence;
    const [transitionState, setTransitionStateInternal] = useState(options.initialState || "exited");
    ...
    useLayoutEffect(() => {
        ...
        const nextDefaultStateInSequence = ["exited", "entering", "entered", "exiting"]
            .map(s => ({ state: s, index: activeSequence.findIndex(s2 => s2 === s) }))
...
function useBodyClickClose(ref, onClose, isActive, toggleElement) {
    const handleBodyClick = (ev) => {
        if (!isTargetInsideContainer(ev.target, ref.current) &&
            !(ev.target === ref.current) &&
            !isTargetInsideContainer(ev.target, toggleElement) // execute if specified toggle element (e.g. menu
...
const useStickyY = (containerRef, stickyElement, shouldBeSticky = true) => {
    useEffect(() => {
        const closestScrollElement = containerRef?.current?.closest(`.${utilityClassScroll}`) || document.body;
        const scrollElement = containerRef?.current?.closest(`.${utilityClassScroll}`) || window;
        const handleScroll = () => {
            ...
            if (classList.includes(utilityClassScrollPadding16)) {
                additionalHeaderHeight = 16 * 4;
            }
            if (classList.includes(utilityClassScrollPadding22)) {
                additionalHeaderHeight = 22 * 4;
            }
            if (!containerRef.current || !stickyElement.current)
                return;
...
const useGridColumns = (deps = []) => {
    const [numGridColumnsSelectors, setNumGridColumnsSelectors] = useState(12);
    const elementRef = useRef(null);
    const viewport = useViewport();
    const viewportGrid = viewport === "xs" || viewport === "sm" ? viewport : "md";
    const maxGridColumns = { xs: 4, sm: 8, md: 12 }[viewportGrid];
    const numGridColumns = Math.min(numGridColumnsSelectors, maxGridColumns);
    const updateGridColumns = () => {
        const columnDefinitionElement = elementRef.current?.closest(`[class*="${hostClass}-column__${viewportGrid}-col-"]`);
        if (columnDefinitionElement) {
            const columnClass = Array.from(columnDefinitionElement.classList).find(c => c.startsWith(`${hostClass}-column__${viewportGrid}-col-`));
            if (columnClass) {
...
export { combineRefs, useBodyClickClose, useDwindle, useGridColumns, useLinkHover, useMenu, usePrevious, useResize, useScroll, useStickyY, useTransition2, useTransitionState, useUniqueId, useViewport, useViewportListener, useViewportRange };
