import { object___lock } from '@lsg/icons';
import { warn } from './LsgConsole.js';

// @ts-strict-ignore
const mapTree = (tree, isLogin, isFk) => {
    const classes = (tree.class || "").split(" ").filter((x) => !!x);
    const { url, id, text, newColumn, noMobile, children = [], params = "", target, targetParams } = tree;
    const result = {
        href: url + params,
        targetValue: id,
        label: text,
...
export { getData, getSingleData, mapTree };
