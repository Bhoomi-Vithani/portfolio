...
import { TextSliderPresentation } from './TextSliderPresentation.js';
...
const TextSliderWrapper = (props) => {
    ...
    const openTextInputAreaButtonRef = useRef(undefined);
    const inputTextLowerRef = useRef(undefined);
    const inputTextUpperRef = useRef(undefined);
    const [state, setState] = useState({
        isSliderVisible: true,
        temporaryInputValue: undefined,
        originalInputValue: props.value,
        isComponentFocused: false,
        isMobile: ["xs"].includes(getInitialViewportSize()), // mobile view with smallest viewport);
    ...
    const onViewportChange = (viewport) => {
        setState(prevState => ({ ...prevState, isMobile: ["xs"].includes(viewport) }));
    };
    useEffect(() => {
        addViewportCallback(onViewportChange);
        return () => removeViewportCallback(onViewportChange);
    }, []);
    ...
    const getInternalInputTextfieldValidation = () => {
        ...
        return { invalidInput, invalidInputField };
    };
    ...
    const handleOnFocus = event => {
        event.preventDefault();
        if (hostRef.current.contains(event.target) && state.isComponentFocused)
            return;
        if (props.onFocus && event.currentTarget.id === hostRef.current.id) {
            setState(prevState => ({ ...prevState, isComponentFocused: true }));
            props.onFocus(event);
        }
    };
    ...
    return (React__default.createElement(TextSliderPresentation, { ...props, id: id, isMobile: state.isMobile, handleOnIconClick: handleOnIconClick, handleOnBlur: handleOnBlur, handleOnFocus: handleOnFocus, handleInputOnFocus: handleInputOnFocus, handleInputOnChange: handleInputOnChange, handleInputOnClear: handleInputOnClear, handleSliderOnChange: handleSliderOnChange, handleTextInputAreaKeydown: handleTextInputAreaKeydown, sliderVisible: state.isSliderVisible, hostRef: hostRef, inputTextLowerRef: inputTextLowerRef, inputTextUpperRef: inputTextUpperRef, openTextInputAreaButtonRef: openTextInputAreaButtonRef, value: value, invalidInput: props.invalidInput || internalInputTextfieldValidation.invalidInput, invalidInputField: props.invalidInputField || internalInputTextfieldValidation.invalidInputField }));
};
