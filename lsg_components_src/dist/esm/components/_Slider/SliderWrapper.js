...
import { extensionKnobIdLower, SliderPresentation } from './SliderPresentation.js';
...
class SliderWrapper extends React__default.Component {
    constructor(props) {
        ...
        this.getLowerValueProp = () => {
            ...
            if (this.props.look === "rangeSlider" && this.props.value[0] <= this.props.value[1])
                return this.props.value[0];
            return this.props.minValue;
        };
        this.getUpperValueProp = () => {
            if (this.props.look === "rangeSlider" && this.props.value[1] >= this.props.value[0])
        ...
        this.registerMouseDownEventListeners = () => {
            Object.keys(this.eventListeners).forEach(key => {
                window.addEventListener(key, this.eventListeners[key], key === "touchmove" || key === "touchend" || key === "touchcancel" ? { passive: true } : undefined);
            });
        };
        ...
        this.getThumbSliderWidth = (slider, asPercent) => {
            const sliderWidth = slider === "lower" || slider === "none"
        ...
        this.onMouseDown = event => {
            ...
            if (event.currentTarget.id) {
                const postfix = event.currentTarget.id.substr(event.currentTarget.id.lastIndexOf(extensionKnobIdLower.at(0)) + 1);
            ...
            this.onDragStart();
            this.setState(() => ({ activeSlider: focusOnSlider, keyboardFocus: "none" }));
        };
        this.onDragStart = () => {
        ...
        this.onTrackClick = event => {
            ...
            this.setState(() => ({ activeSlider: chosenSlider, keyboardFocus: "none" }));
            if (!this.props.onChange) {
                window.requestAnimationFrame(this.resetValueByMissingOnChange);
            }
        };
        this.onFocus = (event, name) => {
            ...
            if (this.props.disabled)
                return;
            const postfix = event.target.id.substr(event.target.id.lastIndexOf(extensionKnobIdLower.at(0)) + 1);
            // code row below checks, whether postfix contains any hint of CurrentSlider-Enum
            if (postfix && !Object.values(["none", "lower", "upper"]).includes(postfix))
                return;
            const focussed = postfix;
            this.setState(() => ({ activeSlider: focussed, keyboardFocus: focussed }));
        ...
        this.handleOnSliderBlur = event => {
            ...
            this.setState(() => ({ activeSlider: "none", keyboardFocus: "none" }));
        };
        ...
        this.onKeyDown = ev => {
            ...
            if (this.props.disabled || ev.key === Key.Tab)
                return;
            ev.preventDefault();
            if (this.state.activeSlider === "none")
                return;
            let percentage;
            ...
            const definedStep = this.props.step || this.props.precision;
            const stepOnSliderInPercent = definedStep
                ? (definedStep / (this.props.maxValue - this.props.minValue)) * 100
                : 1;
            ...
            keyFunctionCollection[Key.ArrowDown] = keyFunctionCollection[Key.ArrowLeft];
            keyFunctionCollection[Key.ArrowUp] = keyFunctionCollection[Key.ArrowRight];
            // Todo (fine tuning): For pageDown/pageUp the slider should move faster.
            keyFunctionCollection[Key.PageDown] = keyFunctionCollection[Key.ArrowLeft];
            keyFunctionCollection[Key.PageUp] = keyFunctionCollection[Key.ArrowRight];
    ...
    shouldComponentUpdate(nextProps) {
        ...
        switch (this.props.look) {
            case "singleSlider":
                ...
                break;
            case "rangeSlider":
                if (currentLower === nextProps.value[0] &&
                    currentUpper === nextProps.value[1]) {
                    break; // no change detect
                }
    ...
    getCalculatedRangeSliderPosition(newPosition, slider, stepSliderInPercent) {
        ...
        if (this.state.activeSlider === "lower") {
            ...
            if (!(this.props.step || this.props.precision)) {
                ...
                percentage =
                    percentage < this.upperSliderThumbPercentage - distanceInPercent
                        ? percentage
                        : this.upperSliderThumbPercentage - distanceInPercent;
            }
        }
        if (this.state.activeSlider === "upper") {
            ...
            if (!(this.props.step || this.props.precision)) {
                // This move should be only done, if a stepper is defined, because this approximation is a coarse tuning
                percentage =
                    percentage > this.lowerSliderThumbPercentage + distanceInPercent
                        ? percentage
                        : this.lowerSliderThumbPercentage + distanceInPercent;
            }
        }
        return percentage;
    }
    render() {
        return (React__default.createElement(SliderPresentation, { ...this.props, activeSlider: this.state.activeSlider, keyboardFocus: this.state.keyboardFocus, sliderMarkerTrackBaseColor: this.state.sliderMarkerTrackBaseColor, lowerSliderThumbPercentage: this.lowerSliderThumbPercentage, upperSliderThumbPercentage: this.upperSliderThumbPercentage, onFocus: this.onFocus, onMouseDown: this.onMouseDown, hostRef: this.hostRef, lowerSliderThumbRef: this.lowerSliderThumbRef, upperSliderThumbRef: this.upperSliderThumbRef, sliderTrackContainerRef: this.sliderTrackContainerRef, onTrackClick: this.onTrackClick, onKeyDown: this.onKeyDown, onHelperMinClick: this.onHelperMinClick, onHelperMaxClick: this.onHelperMaxClick, handleOnSliderBlur: this.handleOnSliderBlur }));
    }
}
