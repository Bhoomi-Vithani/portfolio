...
import { useUniqueId, combineRefs } from '../../utils/Hooks/index.js';
import { Host } from '../../utils/Host.js';
import { HelperTextPresentation } from '../_HelperText/HelperTextPresentation.js';

// @ts-strict-ignore
const TogglePresentation = ({ id: idProp, className, children, noMargin, value, indeterminate, disabled, hasKeyboardFocus, hasMouseHover, invalid, htmlAttrs, label, helperText, errorText, errorTextAriaLive, title, onChange = () => {
    /* empty */
}, name, onKeyboardFocusChange = () => {
    /* empty */
}, onMouseHoverChange = () => {
    /* empty */
}, type, hostClass, inputRef, }) => {
    const id = useUniqueId(`${hostClass}-`, idProp);
    const labelId = `${id}-label`;
    const errorTextId = `${id}-error-text`;
    const helperTextId = `${id}-helper-text`;
    const combinedInputRef = combineRefs(inputRef);
    ...
    return (React__default.createElement(Host, { id: `${id}-base`, className: cleanupClassObject({
        ...
        React__default.createElement("div", { className: cleanupClassObject({
            ...
            React__default.createElement("input", { "aria-describedby": helperText ? helperTextId : undefined, "aria-errormessage": errorText ? errorTextId : undefined, "aria-invalid": !!errorText || !!invalid, ...htmlAttrs, ref: combinedInputRef, className: cleanupClassObject({
                    [`${hostClass}-input`]: true,
                }), id: id, type: type === "switch" ? "checkbox" : type, disabled: disabled, "data-value": value ? "true" : "false", 
                ...
                checked: value || false, value: name, title: title, onChange: e => {
                    e.stopPropagation();
                ...
                }, onBlur: () => onKeyboardFocusChange(false), onMouseEnter: () => {
                    if (document.documentElement.getAttribute("data-whatintent") === "mouse") {
                        onMouseHoverChange(true);
                    }
                }, onMouseLeave: () => onMouseHoverChange(false), role: type === "switch" ? "switch" : undefined }),
            React__default.createElement("div", { className: cleanupClassObject({
                    [`${hostClass}-background`]: true,
                    [`${hostClass}__focus-${type}`]: !disabled && hasKeyboardFocus,
                }) }, type === "switch"
                ? [children, React__default.createElement("div", { className: `${hostClass}-knob`, key: "knob" })]
                : children || React__default.createElement("div", { className: `${hostClass}-knob` })),
            (helperText || errorText || label) && (React__default.createElement("div", { className: `${hostClass}-text` },
                label && (React__default.createElement("label", { id: labelId, className: `${hostClass}-label`, htmlFor: id, onMouseEnter: () => {
                        if (document.documentElement.getAttribute("data-whatintent") === "mouse") {
                            onMouseHoverChange(true);
                        }
                    }, onMouseLeave: () => onMouseHoverChange(false) }, label)),
                React__default.createElement(HelperTextPresentation, { errorTextAriaLive: errorTextAriaLive, helperText: helperText, errorText: errorText, helperTextId: helperTextId, errorTextId: errorTextId, disabled: disabled }))))));
};
TogglePresentation.displayName = "TogglePresentation";

export { TogglePresentation };
