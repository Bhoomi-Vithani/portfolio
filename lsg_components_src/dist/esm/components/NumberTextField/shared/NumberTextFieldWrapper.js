...
import { getActiveLanguage, getDecimalSeparatorByLanguage, getThousandsSeparatorByLanguage } from '../../../utils/Localization.js';
...
const formatNumberToInputString = (value, decimalDigits) => {
    ...
    return result.replace(".", getDecimalSeparatorByLanguage(getActiveLanguage()));
};
const normalize = (value, maxDecimalDigits) => {
    const thousandsSeparator = getThousandsSeparatorByLanguage(getActiveLanguage());
    let normalizedStringValue = value
        .replace(/[^\d,.+-]/g, "")
        .replaceAll(thousandsSeparator, "")
        .replace(/,/g, "."); // remove none number format elements
    ...
    const normalizedString = normalizedStringValue.replace(/\./g, getDecimalSeparatorByLanguage(getActiveLanguage()));
    return {
        normalizedNumber: normalizedStringValue === "" ? NaN : Number(normalizedStringValue), // 123123.123
        normalizedString,
    };
};
const defaultValueNumberFormatter = (v, numberDecimalPlaces) => new Intl.NumberFormat(getActiveLanguage(), {
    minimumFractionDigits: numberDecimalPlaces,
    maximumFractionDigits: numberDecimalPlaces ?? 20,
}).format(v);
const NumberTextFieldWrapper = ({ valueNumberFormatter = defaultValueNumberFormatter, value, onChange, onFocus, onBlur, htmlAttrs = {}, decimalDigits, spacing, ...otherProps }) => {
    ...
    const stringValue = isNumber(value)
        ? valueNumberFormatter(value, decimalDigitsRegulated)
        : value;
    ...
    const handleChange = (v, name, ev) => {
        const { normalizedNumber } = normalize(v.toString(), decimalDigitsRegulated);
    ...
    return (React__default.createElement(TextFieldWrapper, { ...otherProps, onKeyDown: ev => {
        ...
        value: inputValue ?? stringValue, onFocus: handleFocus, onBlur: handleBlur, onChange: handleChange, htmlAttrs: {
            ...htmlAttrs,
            inputMode: htmlAttrs.inputMode ?? "decimal",
        }, spacing: effectiveSpacing }));
};

export { NumberTextFieldWrapper };
