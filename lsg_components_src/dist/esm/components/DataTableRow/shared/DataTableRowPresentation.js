...
import { utilityClassesBackgroundBaseAfter, utilityClassesBackgroundHoverAfter, utilityClassesBackgroundFocusAfter } from '../../../styles/utilityClassesColor.styles.js';
...
import { DataTableRowTogglePresentation } from './DataTableRowToggle/DataTableRowTogglePresentation.js';

// @ts-strict-ignore
/* eslint-disable no-param-reassign */
const DataTableRowPresentation = (props) => {
    const { data, colspans, className, inputName, visibility, isHeader, isTopHeader, columnsPreprocessed, inputType, inputValue, onInputChange, htmlInputAttrs = {}, ariaAttrs, isExpandableHeader, ids, id, onLinkClick, linkHref, htmlLinkAttrs, htmlCellAttrs, hasMouseHover, hasMouseHoverToggle, onMouseHoverChange, onMouseHoverChangeToggle, hasKeyboardFocus, hasKeyboardFocusToggle, onKeyboardFocusChange, onKeyboardFocusChangeToggle, gridStructure, inputHidden, inputDisabled, separateColumnsXsWidthRatio, tableCellsArrayRef, toggleColumnCellRef, insetLevel, spacing, } = props;
    ...
    const dataVisiblePre = columnsPreprocessed.reduce((acc, colHeader, index) => {
        ...
        if (acc.span > 1) {
            ...
            if (colHeader.isLabelColumn && hasRowAction) {
                contents = (React__default.createElement(ActionPresentation, { id: `${id}-label`, className: `${hostClass}-tr-label-column-action`, ref: actionRef, userSelect: true, onClick: (e, name) => {
                    ...
                    }, href: linkHref, htmlAttrs: htmlLinkAttrs, onMouseHoverChange: onMouseHoverChange, onKeyboardFocusChange: onKeyboardFocusChange }, contents));
                foundLabel = true;
            }
            else if (colHeader.isLabelColumn && hasLabel) {
                contents = (React__default.createElement(LabelPresentation, { id: `${id}-label`, className: `${hostClass}-tr-label-column-label`, htmlFor: (hasToggleColumn && `${id}-input`) || undefined, expandToOverlay: !inputDisabled, onMouseHoverChange: onMouseHoverChangeToggle, onKeyboardFocusChange: onKeyboardFocusChangeToggle }, contents));
                foundLabel = true;
            }
        ...
        hasLabel: false,
        dataIndex: 0,
        spannedColHeaderIndexes: [], // a list of data columns that is skipped by the colspan prop.
        labelColumn: NaN,
    });
    ...
    const gridHeaderColSpanInfos = gridStructure.gridHeaderCols.map(dataCols => {
        const result = { dataCols, isSpanned: true, headerColspan: 1 };
    ...
    const rowSeparatorStyle = gridStructure.tableRowStyles.separatorStyles || { weight: "default" };
    ...
    return (React__default.createElement("tr", { id: id, className: cleanupClassObject({
            [className]: !!className,
            [`${hostClass}-tr`]: true,
            [`${hostClass}-tr__row`]: !isExpandableHeader && !isTopHeader,
            [`${hostClass}-tr__row-separator__${rowSeparatorStyle.weight}`]: true,
            [`${hostClass}-tr__${visibility}`]: visibility,
            [`${hostClass}-tr__expandable`]: isExpandableHeader,
            [`${hostClass}-tr__disabled`]: inputDisabled,
            [`${hostClass}-tr__inset-level-${insetLevelClassText}`]: true,
            [`${hostClass}-tr__spacing-${spacing}`]: !!spacing && !isHeader,
            [`${hostClass}-tr__has-row-action`]: hasRowAction,
            [utilityClassesBackgroundBaseAfter]: !isHeader,
            [utilityClassesBackgroundHoverAfter]: hasMouseHover,
            [utilityClassesBackgroundFocusAfter]: hasKeyboardFocus,
        }), ref: tableRowRef, 
        ...
        style: { position: "relative", ...gridStructure.tableRowStyles }, ...ariaAttrs },
        hasToggleColumn && (React__default.createElement(DataTableRowTogglePresentation, { ...htmlCellAttrs, id: `${id}-input`, as: TX, toggleAs: (hasCheckbox && CheckboxPresentation) || (hasRadio && RadioPresentation), inputType: inputType, inputDisabled: inputDisabled, inputHidden: inputHidden, value: inputValue, name: inputName, onChange: onInputChange, htmlAttrs: htmlInputAttrs, hasMouseHover: hasMouseHoverToggle, hasKeyboardFocus: hasKeyboardFocusToggle, onMouseHoverChange: onMouseHoverChangeToggle, onKeyboardFocusChange: onKeyboardFocusChangeToggle, className: cleanupClassObject({
                [`${hostClass}-tx-data-separated-toggle-column`]: dataSeparated.length > 0,
                [`${hostClass}__has-top-margin`]: true,
            }), gridStructure: gridStructure, isHeader: isHeader, cellRef: toggleColumnCellRef })),
        dataVisiblePre.columns.map(col => {
            ...
            if (!col.isSeparated && !(isXsViewport && col.isActionColumn)) {
                ...
                if (isValidElement(col.contents) && typeof col.contents !== "string") {
                    inner = fragmentMap(col.contents, (child) => React__default.cloneElement(child, header.horizontalAlignment && typeof child.type !== "string"
                        ? {
                ...
                const styleLeft = validRatio && !col.isActionColumn ? { flex: separateColumnsXsWidthRatio } : {};
                const styleRight = validRatio && !col.isActionColumn ? { flex: 1 - separateColumnsXsWidthRatio } : {};
                // separated label is aria-hidden because in the table header (th) the col.contents = col.title.
                // it would be read twice.
                inner = (React__default.createElement(React__default.Fragment, null,
                    React__default.createElement("div", { className: `${hostClass}-separated-label`, "aria-hidden": true, style: styleLeft }, col.title),
                    React__default.createElement("div", { className: `${hostClass}-separated-contents`, style: styleRight }, col.contents)));
            }
            const gridHeaderColIndex = hasToggleColumn
                ? header.gridHeaderColIndex + 2
                : header.gridHeaderColIndex + 1;
            ...
            return (React__default.createElement(React__default.Fragment, { key: headerIndex },
                React__default.createElement(TX, { ...htmlCellAttrs?.[headerIndex], id: ids?.[headerIndex], className: cleanupClassObject({
                        [`${hostClass}-tx`]: true,
                        [`${hostClass}-${TX}`]: true,
                        [`${hostClass}__wrap-no-wrap`]: col.textWrap === "no-wrap",
                        [`${hostClass}__wrap-hyphenation`]: header.textWrap === "hyphenation",
                        [`${hostClass}__wrap-character`]: header.textWrap === "character",
                        [`${hostClass}__first-column`]: !hasToggleColumn && header.isFirstGridCol,
                        [`${hostClass}__first-data-column`]: header.gridHeaderColIndex === 0,
                        [`${hostClass}__last-column`]: header.isLastGridCol,
                        [`${hostClass}__${header.horizontalAlignment}`]: (header.horizontalAlignment && header.horizontalAlignmentType !== "prop-only") ||
                            isTopHeader,
                        [`${hostClass}-th__hidden`]: isTopHeader && header.headerHidden,
                        [`${hostClass}__has-top-margin`]: header.hasTopMargin,
                        [`${hostClass}__has-bottom-margin`]: (header.hasBottomMargin || isExpandableHeader) &&
                            !(header.isActionColumn && !isXsViewport),
                        [`${hostClass}__separated`]: header.isSeparated,
                        [`${hostClass}__first_separated_column`]: col.isFirstSeparatedColumn,
                        [`${hostClass}__action-column`]: header.isActionColumn,
                        [`${hostClass}__separated-action-column`]: isXsViewport && header.isActionColumn,
                        [`${hostClass}__sortable-column`]: header.isSortable,
                        [`${hostClass}__grid-col-${header.gridHeaderColIndex}`]: true,
                        [`${hostClass}__sticky-left`]: header.isStickyLeft,
                        [`${hostClass}__sticky-right`]: header.isStickyRight,
                    }), style: cellStyle, scope: isHeader ? "col" : undefined, ref: ref => {
                        if (tableCellsArrayRef?.current)
                            tableCellsArrayRef.current[headerIndex] = ref;
                    }, "aria-sort": isHeader ? col.ariaSort : undefined }, inner && React__default.createElement("div", { className: `${hostClass}-tx-inner` }, inner)),
                ...
                col.tableColSpan > 1 &&
                    Array(col.tableColSpan - 1)
                        .fill(0)
                        .map((_, i) => React__default.createElement(TX, { key: `${headerIndex}-${i}` }))));
        })));
};
DataTableRowPresentation.displayName = "DataTableRowPresentation";

export { DataTableRowPresentation };
