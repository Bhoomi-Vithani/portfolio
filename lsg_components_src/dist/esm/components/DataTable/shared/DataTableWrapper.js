import React__default, { useRef, useState, useCallback, useEffect, cloneElement } from 'react';
import { useViewport, useScroll, useUniqueId } from '../../../utils/Hooks/index.js';
import { fragmentMap } from '../../../utils/ReactUtils.js';
import { hostClass as hostClass$1 } from '../../PortalHeader/shared/PortalHeader.styles.js';
import { mainClass } from '../../_HeaderContainer/HeaderContainer.styles.js';
import { hostClass } from './DataTable.styles.js';
import { defaultProps, DataTablePresentation } from './DataTablePresentation.js';

// @ts-strict-ignore
const getHeaderOffset = (viewport) => {
    const elements = document.getElementsByClassName(`${mainClass}__sticky`);
    if (elements?.length > 0) {
        const portalHeader = document.getElementsByClassName(hostClass$1)?.length > 0;
        return ["lg", "xl"].includes(viewport) && portalHeader
            ? elements[0].clientHeight / 2 // full header become reduce to half height, when it's a portal header and become scrolled
            : elements[0].clientHeight; // has reduced header height
    }
    return 0;
};
const DataTableWrapper = (props) => {
    const { id: idProp, radiosName, onRadiosChange, radiosValue, labelColumn, columns, inputType, children, footer, isHeaderSticky, sortColumn, sortColumns: sortColumnsProp, sortAscending: sortAscendingProp, width, separateColumnsXsWidthRatio, insetLevel, ...otherProps } = { ...defaultProps, ...props };
    const tableHeadRef = useRef(undefined);
    ...
    const onHandleScroll = useCallback(() => {
        if (isHeaderSticky) {
            ...
            if (currentTopPosition > state.headerOffsetOn && state.headerShadow) {
                ...
                setState(prevState => ({ ...prevState, headerShadow: false }));
            }
            if (currentTopPosition <= state.headerOffsetOn && !state.headerShadow) {
                // set shadow on sticky table header
                setState(prevState => ({ ...prevState, headerShadow: true }));
            }
        }
    }, [state, setState]);
    useScroll(onHandleScroll);
    useEffect(() => {
        if (isHeaderSticky) {
            setState(prevState => ({ ...prevState, headerOffsetOn: getHeaderOffset(viewport) }));
        }
    }, []);
    useEffect(() => {
        if (isHeaderSticky) {
            ...
            if (offset > 0 && offset !== state.headerOffsetOn && ["lg", "xl"].includes(viewport)) {
                setState(prevState => ({ ...prevState, headerOffsetOn: offset }));
            }
        }
    ...
    const sortColumns = (typeof sortColumn === "number" && sortColumn > -1 && [sortColumn]) ||
        (Array.isArray(sortColumnsProp) && sortColumnsProp) ||
        [];
    const sortAscending = (typeof sortAscendingProp === "boolean" && [sortAscendingProp]) ||
        (Array.isArray(sortAscendingProp) && sortAscendingProp) ||
        [];
    ...
    const gridStructure = columnsPreprocessed.reduce((acc, colPreprocessed, i) => {
        ...
        else if (result.gridHeaderCols.length === 0 ||
            !(colPreprocessed.isWrapped || colPreprocessed.isHiddenInHeader)) {
        ...
        gridHeaderCols: [], // a structure of headerIndexes, will look something like this: [[(if checkbox
        // exists)], [0,1,2], [3], [4,5,8]]
        gridSeparatedCols: [], // an array of the indexes of the separated cols e.g. [6,7]
        gridSeparatedActionColIndex: undefined, // the action col in xs viewport
    ...
    if (gridStructure.gridHeaderCols.length > 0) {
        for (let col = 0; col < gridStructure.gridHeaderCols.length; col++) {
            ...
            gridStructure.gridHeaderCols[col].forEach(headerIndex => {
                colIsRightAligned =
                    colIsRightAligned || columnsPreprocessed[headerIndex].horizontalAlignment === "right";
            });
    ...
    gridTemplateColumns += columnsPreprocessed
        .filter(col => !col.isHiddenInHeader)
        .reduce((acc, val) => val.isWrapped
    ...
    const elementMapper = (isFooter) => (child, i) => {
        ...
        return cloneElement(rowChild, {
            ...
            onInputChange: inputType === "radio"
                ? (_, _1, event) => onRadiosChange(rowChild.props.inputName, radiosName, event)
                : rowChild.props.onInputChange,
        });
    };
    return (React__default.createElement(DataTablePresentation, { ...otherProps, id: id, inputType: inputType, columnsPreprocessed: columnsPreprocessed, gridStructure: gridStructure, columnsSeparated: columnsSeparated, footer: footer && fragmentMap(footer, elementMapper(true)), tableHeadRef: tableHeadRef, tableHeadStickPosition: state.headerOffsetOn, headerShadow: state.headerShadow, isHeaderSticky: isHeaderSticky, isMultiSort: isMultiSort, sortColumns: sortColumns, sortAscending: sortAscending, width: width }, fragmentMap(children, elementMapper(false))));
};
