...
import { getInitialViewportSize, addResizeCallback, addViewportCallback, removeResizeCallback, removeViewportCallback } from '../../../utils/windowEvents/ResizeEvents.js';
import { setLsgTimeout } from '../../../utils/timing.js';
import '../../ChartInfos/ChartInfo.js';
import { mapChartInfoPosition } from '../../ChartInfos/shared/ChartInfoContainer.js';
import { hostClass } from './CircleCountdown.styles.js';
import { defaultMinChartSize, defaultMaxChartSize, defaultDelayStartTimer, CircleCountdownPresentation, defaultProps } from './CircleCountdownPresentation.js';
...
const getSpaceFactor = (space, position, viewport) => {
    ...
    return [
        { space: "outer", vport: "desktop", pos: "top-bottom", factor: 1.4 },
        { space: "outer", vport: "mobile", pos: "top-bottom", factor: 1.5 },
        { space: "outer", vport: "desktop", pos: "right", factor: 1.5 },
        { space: "outer", vport: "mobile", pos: "right", factor: 1.5 },
        { space: "offset", vport: "desktop", pos: "right", factor: 0.5 },
        { space: "offset", vport: "mobile", pos: "right", factor: 0.5 },
        { space: "offset", vport: "desktop", pos: "top-bottom", factor: 0.4 },
        { space: "offset", vport: "mobile", pos: "top-bottom", factor: 0.5 },
...
class CircleCountdownWrapper extends React__default.Component {
    constructor(props) {
        ...
        this.getCurrentRectWidthForChart = (viewPortSize) => {
            ...
            if (this.props.sizeMode === "fixed") {
                clientRectWidth =
                    this.host.clientWidth < defaultMinChartSize || ["xs"].includes(viewPortSize)
                        ? defaultMinChartSize
                        : defaultMaxChartSize;
            }
            else {
        ...
        this.startTimer = (restart) => {
            ...
            this.countdown = window.setInterval(() => {
                this.setState(previousState => ({ secondsLeft: previousState.secondsLeft - 0.05 }));
                if (this.state.secondsLeft <= 0) {
                    clearInterval(this.countdown);
                    this.setState(() => ({
                        errorText: this.props.countDownFinishText || texts.lsg.component.CircleDiagram.errorFinishCountdown,
                    }));
                }
            }, 50);
        };
        this.updateSize = (newWidth) => {
            ...
            let areaPosition = mapChartInfoPosition(this.props.chartInfo?.position, viewPortSize);
            ...
            this.setState(() => ({
                size: {
                    ...
                    heightContainer: this.props.chartInfo &&
                        ["top", "bottom"].includes(areaPosition) &&
                        [defaultMinChartSize, defaultMaxChartSize].includes(clientRectWidth)
                        ? Math.round(clientRectWidth * getSpaceFactor("outer", "top-bottom", viewPortSize)) /* creates offset for chart info on top or bottom */
                        : clientRectWidth,
                    widthContainer: this.props.chartInfo &&
                        areaPosition === "right" &&
                        [defaultMinChartSize, defaultMaxChartSize].includes(clientRectWidth)
                        ? Math.round(clientRectWidth * getSpaceFactor("outer", "right", viewPortSize)) /* creates offset for chart info on the right */
                        : clientRectWidth,
                },
            }));
        ...
        this.animate = () => {
            ...
            const valueLabel = this.props.valueAnimationCallback && animationAmount < 1
            ...
            if (this.props.amount && this.props.amount * animationAmount < this.props.amount) {
                this.setState(() => ({
                    ...
                    setLsgTimeout(this.animate);
                });
            }
            else {
                this.setState({ timestampStarted, valueLabel, animationAmount });
            }
        };
    ...
    componentDidMount() {
        ...
        const options = {
            ...
            threshold: 0.4, // 0-1 how much of element needs to be shown
            rootMargin: "0px", // px / %
        };
        ...
        // 5 Second countdown
        if (this.props.countdown) {
            setTimeout(() => this.startTimer(), defaultDelayStartTimer * 1.5);
        }
    ...
    componentDidUpdate(prevProps) {
        ...
        if (this.state.secondsLeft > 20 && this.readLabelAfterRefresh)
            this.readLabelAfterRefresh = false;
    ...
    render() {
        ...
        const innerSquareSize = Math.min(this.props.chartInfo && ["top", "bottom"].includes(this.state.size.chartPosition)
            ? Math.round(this.state.size.heightContainer /
                getSpaceFactor("outer", "top-bottom", this.state.viewPort) /* 1.4 */ // calc chart size depending of viewport
            )
            : this.state.size.heightContainer, this.props.chartInfo && this.state.size.chartPosition === "right"
            ? Math.round(this.state.size.widthContainer / getSpaceFactor("outer", "right", this.state.viewPort) /* 1.5 */ // calc chart size depending of viewport
            )
        ...
        const finalSizing = {
            ...
            this.props.chartInfo && ["top", "bottom"].includes(this.state.size.chartPosition)
                ? Math.round((this.state.size.heightContainer /
                    getSpaceFactor("outer", "top-bottom", this.state.viewPort)) *
                    getSpaceFactor("offset", "top-bottom", this.state.viewPort))
                : this.state.size.heightContainer,
            widthChartInfo: 
            // calc the space of chart info depending on chart info position
            this.props.chartInfo && this.state.size.chartPosition === "right"
                ? Math.round((this.state.size.widthContainer / getSpaceFactor("outer", "right", this.state.viewPort)) *
                    getSpaceFactor("offset", "right", this.state.viewPort))
                : this.state.size.widthContainer,
        };
        const valueLabel = this.state.valueLabel || this.props.valueLabel;
        const amount = ["animated"].includes(this.getPresentationMode()) &&
            this.props.amount * this.state.animationAmount < this.props?.amount
            ? this.props.amount * this.state.animationAmount
            : this.props.amount; // PresentationMode === "static, countdown process value become managed in Presentation in conjunction with startTimer
        return (React__default.createElement(CircleCountdownPresentation, { ...this.props, valueLabel: valueLabel, size: finalSizing, amount: amount, hostRef: hostRef => {
                this.host = hostRef;
            }, secondsLeft: this.state.secondsLeft, errorText: this.state.errorText, headlineRef: r => {
                this.headline = r;
            }, ctaBelowCircle: this.state.ctaBelowCircle, startTimer: restart => this.startTimer(restart), presentationMode: this.getPresentationMode(), readLabelAfterRefresh: this.readLabelAfterRefresh }));
    }
}
CircleCountdownWrapper.defaultProps = defaultProps;
