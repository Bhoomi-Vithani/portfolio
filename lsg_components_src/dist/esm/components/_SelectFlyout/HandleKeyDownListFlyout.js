...
const getPageScrollNumItems = (direction, host, focussedElement) => {
    ...
    function nextSibling(el) {
        ...
        return direction === "up"
            ? getElement(ele => ele.previousElementSibling)
            : getElement(ele => ele.nextElementSibling);
    }
...
const addTypeAhead = (char, options, focussedValue, onFocusValue) => {
    ...
    clearLsgTimeout(typeAheadTimeout);
    ...
    if (optIndex !== -1) {
        ...
        if (before === last) {
            ...
            if (filteredOptions.length > 0) {
                const filteredIndex = focussedValue && options[focussedValue]?.label
                    ? filteredOptions.findIndex(o => o.label === options[focussedValue].label)
                    : 0;
                onFocusValue(options.findIndex(o => o.label ===
                    filteredOptions[filteredIndex >= filteredOptions.length - 1 ? 0 : filteredIndex + 1].label));
            }
        }
    }
    typeAheadTimeout = setLsgTimeout(() => (typeAhead = ""), 1000);
};
const isLetterCharacter = (char) => /^[A-Za-z0-9-_äöüß]$/i.test(char);
function handleKeyDown({ event, options, focussedIndex, onChange, onFocusIndex, flyoutElement, focussedElement, onClose, useTypeAhead, isTextInputSelect, }) {
    ...
    const keyFunction = {
        ...
        [Key.ArrowUp]: () => (selectedIndex > 0 ? selectedIndex - 1 : 0),
        // a11y pattern says: "If focus is on the last option, either returns focus to the combobox or does
        // nothing." We do nothing. https://www.w3.org/WAI/ARIA/apg/patterns/combobox/
        [Key.ArrowDown]: () => (selectedIndex < length - 2 ? selectedIndex + 1 : length - 1),
        [Key.PageUp]: () => Math.max((selectedIndex >= 0 ? selectedIndex : 0) - getPageScrollNumItems("up", flyoutElement, focussedElement), 0),
        [Key.PageDown]: () => Math.min((selectedIndex >= 0 ? selectedIndex : 0) +
            getPageScrollNumItems("down", flyoutElement, focussedElement), length - 1),
        [Key.Enter]: confirmFn,
        [Key.Escape]: () => {
            onClose(event);
        },
    ...
    if (keyFunction && !(isTextInputSelect && event.key === Key.Space)) {
...
function handleKeydownListFlyout({ event, open, focussedIndex, options, onChange, onOpen, onClose, onFocusIndex, flyoutElement, focussedElement, afterAction = () => { }, useTypeAhead, isTextInputSelect, }) {
    if (event.shiftKey && event.key === Key.Shift) {
        return;
    }
    ...
    else if ([Key.Home, Key.End].includes(event.key) || isLetterCharacter(event.key)) {
        onOpen();
        handleKeyDownFn();
    }
    else if ([Key.ArrowUp, Key.ArrowDown, Key.Enter, Key.Space].includes(event.key)) {
        onOpen();
    }
    afterAction();
}

export { handleKeyDown, handleKeydownListFlyout };
