...
function getViewportMediaQueryRange(minViewport, maxViewport) {
    return [
        minViewport && `(min-width: ${pxToRem(breakPointsMin[minViewport])}rem)`,
        maxViewport && breakPointsMax[maxViewport] && `(max-width: ${pxToRem(breakPointsMax[maxViewport])}rem)`,
    ]
        .filter(x => !!x)
        .join(" and ");
}
...
function getViewportMediaQuery(viewport, direction) {
    const min = direction === "min" || direction === "exact" ? breakPointsMin[viewport] : undefined;
    const max = direction === "max" || direction === "exact" ? breakPointsMax[viewport] : undefined;
    return [min ? `(min-width: ${pxToRem(min)}rem)` : "", max ? `(max-width: ${pxToRem(max)}rem) ` : ""]
        .filter(mq => !!mq)
        .join(" and ");
}
