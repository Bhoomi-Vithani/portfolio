...
class TransitionHandler {
  constructor() {
    ...
    this.makeTransitionState = init => ({
      ...Object.keys(STATES).filter(state => state !== init).reduce((acc, curr) => ({
        ...acc,
        [curr]: false
      }), {}),
      [init]: true
    ...
    this.transition = (isEnter, timeout, prevState, onChange = () => {
      ...
      const saveTimeout = typeof timeout === "number" ? {
        enter: timeout,
        exit: timeout
      } : timeout;
      const performEnter = () => {
        ...
        this.timeout = timing.setLsgTimeout(() => {
          onChange(this.makeTransitionState(STATES.entered));
        }, saveTimeout.enter);
      };
      const performExit = () => {
      ...
      if (isEnter) {
        if (prevState.unmounted) {
          onChange(this.makeTransitionState(STATES.exited));
          this.timeout = timing.setLsgTimeout(performEnter, 0);
        }
