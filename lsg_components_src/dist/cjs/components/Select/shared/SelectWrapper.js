...
var SelectPresentation = require('./SelectPresentation.js');
...
const SelectWrapper = externalProps => {
  const props = FunctionalHelpers.functionalHelpers(externalProps, defaultProps);
  const [isOpen, setIsOpen] = React__default.useState(false);
  const [currentValue, setCurrentValue] = React__default.useState(props.value);
  const [displayValue, setDisplayValue] = React__default.useState(props.options.find(o => o.value === props.value)?.label || "");
  const [focussedValue, setFocussedValue] = React__default.useState();
  const [scrollFocussedElementIntoView, setScrollFocussedElementIntoView] = React__default.useState();
  const hostRef = React__default.useRef(undefined);
  const focussedElementRef = React__default.useRef(undefined);
  const containerElementRef = React__default.useRef(undefined);
  const flyoutElementRef = React__default.useRef(undefined);
  const currentPropsRef = React__default.useRef(props);
  ...
  const handleBlur = (ev, name) => {
    ...
    if (!currentProps.withTextInput && (DomUtils.isTargetInsideContainer(target, hostRef.current) || DomUtils.isTargetInsideContainer(target, flyoutElementRef.current)) || target === hostRef.current) {
      return;
    }
  ...
  const handleDisplayValueChange = value => {
    const option = value ? props.options.find(o => {
      switch (props.searchFunction) {
        ...
        case "startOfWord":
          return o.label.toLowerCase().split(" ").some(word => word.startsWith(value.toLowerCase()));
        default:
          return false;
      }
    }) : props.options[0];
  ...
  return /*#__PURE__*/React__default.createElement(SelectPresentation.SelectPresentation, {
    ...otherProps,
    onFlyoutChange: handleFlyoutChange,
    flyoutValue: value,
    open: isOpen,
