...
var TabBarPresentation = require('./TabBarPresentation.js');
...
const TabBarWrapper = props => {
  ...
  const innerElement = React__default.useRef(null);
  const activeElement = React__default.useRef(null);
  const [isDragging, setIsDragging] = React__default.useState(false);
  const [offsetLeft, setOffsetLeft] = React__default.useState(0);
  const [, setCounter] = React__default.useState(1);
  const [mouseDownXPos, setMouseDownXPos] = React__default.useState(0);
  const [initialOffset, setInitialOffsetLeft] = React__default.useState(0);
  ...
  React__default.useEffect(() => {
    ...
    return () => {
      ...
      ResizeEvents.removeResizeCallback(onResize);
  ...
  return /*#__PURE__*/React__default.createElement(TabBarPresentation.TabBarPresentation, {
    ...props,
    onChange: (newName, e) => {
      if (!isDragging) {
        onChange(newName, e);
    ...
    centeredLayout: tabsCentered,
    onNewElementFocussed: elementToFocus => {
      /**
...
exports.TabBarWrapper = TabBarWrapper;
