...
var GallerySliderPresentation = require('./GallerySliderPresentation.js');
...
class GallerySliderWrapper extends React__default.Component {
  constructor(props) {
    ...
    this.getSlideByPage = page => {
      ...
      const pageCount = Math.ceil(ReactUtils.fragmentCount(this.props.children) / slidesPerPage);
    ...
    Math.max(ReactUtils.fragmentCount(this.props.children) - slidesPerPage, 0);
    ...
    this.onMouseMove = e => {
      ...
      if (this.state.mouseMovePosition === undefined && Math.abs(this.state.mouseDownPositionX - screenX) < Math.abs(this.state.mouseDownPositionY - screenY)) {
        this.removeSwipeEventListeners();
        this.setState(() => ({
    ...
    this.setNextPage = e => {
      const pageCount = Math.ceil(ReactUtils.fragmentCount(this.props.children) / this.state.slidesPerPage);
    ...
    this.updateSlideWidth = () => {
      const slidesPerPage = this.getSlidesPerPage(ResizeEvents.getViewportSize());
    ...
    const slidesPerPage = this.getSlidesPerPage(ResizeEvents.getInitialViewportSize());
  ...
  componentDidMount() {
    ResizeEvents.addResizeCallback(this.updateSlideWidth);
  }
  componentWillUnmount() {
    ResizeEvents.removeResizeCallback(this.updateSlideWidth);
    this.removeSwipeEventListeners();
  }
  ...
  render() {
    ...
    const slides = ReactUtils.fragmentCount(children);
    const dotCount = slidesPerPage !== 0 ? Math.ceil(slides / slidesPerPage) : 0;
    const activeDot = Math.ceil(currentSlide / slidesPerPage);
    return /*#__PURE__*/React__default.createElement(GallerySliderPresentation.GallerySliderPresentation, {
      ...this.props,
      someElementRef: r => this.someInnerElement = r,
      outerRef: r => this.outerElement = r,
      currentSlide: currentSlide,
      animation: animation,
      mouseDownPositionX: mouseDownPositionX,
      mouseDownPositionY: mouseDownPositionY,
      mouseDownTimestamp: mouseDownTimestamp,
      mouseMovePosition: mouseMovePosition,
      onMouseDown: e => this.onMouseDown(e),
      onDragStart: e => e.preventDefault(),
      handleArrowRightClick: e => this.setNextPage(e),
      handleArrowLeftClick: e => this.setPreviousPage(e),
      handlePointClick: this.setPage,
      dotCount: dotCount,
      activeDot: activeDot,
      translate: translate,
...
exports.GallerySliderWrapper = GallerySliderWrapper;
