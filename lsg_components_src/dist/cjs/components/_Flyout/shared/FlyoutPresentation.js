...
var Drawer_styles = require('../../Drawer/shared/Drawer.styles.js');
var LayerPresentation = require('../../Layer/shared/LayerPresentation.js');
var Section_styles = require('../../Section/shared/Section.styles.js');
var ThemePresentation = require('../../Theme/shared/ThemePresentation.js');
var Flyout_styles = require('./Flyout.styles.js');
...
const getRects = (toggleElement, portalElement) => {
  ...
  const innerSectionElement = toggleElement?.closest(`.${Section_styles.hostClass}`)?.firstChild || document.documentElement;
...
const FlyoutPresentation = /*#__PURE__*/React__default.forwardRef(({
  id,
  children,
  className,
  noMargin,
  onKeyDown = () => {},
  toggleElement: toggleElementProp,
  toggleContainerElement: toggleContainerElementProp,
  hasTabIndex = true,
  noAutoFocus = false,
  innerSpacing = "none",
  ...
  const inlineRef = React__default.useRef(null);
  ...
  const closestScrollElement = toggleElement?.closest(`.${utilityClassesScroll_styles.utilityClassScroll}`);
  const scrollElement = closestScrollElement || document.body;
  const scrollPosition = closestScrollElement?.scrollTop ?? window.scrollY;
  const internalRef = index.combineRefs(hostRef, ref);
  index.useBodyClickClose(internalRef, onClose, open, toggleContainerElement);
  // TODO distinct between dialog and combobox flyout
  const close = ev => {
    if (DomUtils.isTargetInsideContainer(ev.target, toggleContainerElement)) {
      // Ignore if click is on the Button that opened the flyout
      return;
    }
    onClose(ev);
    if (DomUtils.isTargetInsideContainer(document.activeElement, internalRef.current) || document.activeElement === internalRef.current) {
      toggleElement?.focus();
    }
  };
  // TODO distinct between dialog and combobox flyout
  ...
  const handleKeyDown = ev => {
    if (ev.key === Keys.Key.Escape) {
      close(ev);
    } else if (ev.key === Keys.Key.Tab && hasTabIndex) {
      onClose(ev);
    } else {
      onKeyDown(ev);
    }
  ...
  const [transitionState, onTransitionEnd] = index.useTransition2(open, {
   ...
   */);
  const isInsideLayer = () => {
    const rightDrawer = toggleElement.closest(`.${Drawer_styles.hostClass}__right`);
    const leftDrawer = toggleElement.closest(`.${Drawer_styles.hostClass}__left`);
    return rightDrawer ? "right" : leftDrawer ? "left" : false;
  };
  const [height, setHeight] = React__default.useState(undefined);
  const [placeAbove, setPlaceAbove] = React__default.useState(undefined);
  let styles = {};
  const {
    usableRect,
    sectionRect,
    portalRect
  } = getRects(toggleElement, scrollElement);
  const [maxWidthState, setMaxWidthState] = React__default.useState(Math.max(usableRect.width || 0, maxWidth || 0) || undefined);
  const [widthState, setWidthState] = React__default.useState(internalRef?.current?.getBoundingClientRect()?.width);
  const [gotFocus, setGotFocus] = React__default.useState(false);
  ...
  const updateDimensions = () => {
    if (transitionState === "initial-hidden" || transitionState === "entered") {
      ...
      if (isInsideLayer()) {
        setMaxWidthState(Math.max(getRects(toggleElement, scrollElement).sectionRect.width || 0, maxWidth || 0) || undefined);
      } else {
        setMaxWidthState(Math.max(getRects(toggleElement, scrollElement).usableRect.width || 0, maxWidth || 0) || undefined);
      }
    }
  };
  index.useResize(updateDimensions, [widthState]);
    ...
    if (placeAbove !== undefined && transitionState === "entered" && !gotFocus) {
      ...
      if (!isInView) {
        internalRef.current.scrollIntoView({
          ...
          behavior: animation.isPreferReducedMotion() ? "instant" : "smooth"
        });
      }
    }
  ...
  if (typeof window !== "undefined" && toggleContainerElement && height) {
    const isRelative = window.getComputedStyle(scrollElement).position === "relative" || window.getComputedStyle(scrollElement).position === "absolute" ||
    ...
    if (placeAbove) {
      ...
      const offsetTop = isRelative ? portalRect.top - scrollElement.parentElement.getBoundingClientRect().top : 0;
    ...
    if (widthState) {
      const enoughSpaceRightSection = () => {
        if (sectionRect.right - rectToggle.left >= widthState) {
          ...
          return {
            left: rectToggle.left - portalRect.left
          };
        }
        return false;
      };
      const enoughSpaceLeftSection = () => {
        if (rectToggle.right - sectionRect.left >= widthState) {
          ...
          return {
            right: portalRect.right - rectToggle.right
          };
        }
        return false;
      };
      ...
      const checks = preferOpenToLeft ? [enoughSpaceRightSection, enoughSpaceLeftSection, enoughSpaceRightScreen, enoughSpaceLeftScreen] : [enoughSpaceLeftSection, enoughSpaceRightSection, enoughSpaceLeftScreen, enoughSpaceRightScreen];
      ...
      if (typeof newStyles !== "boolean") {
        ...
        const isRatherRight = rectToggle.left - sectionRect.left > sectionRect.right - rectToggle.right || isInsideLayer() === "right";
        if (isRatherRight) {
  ...
  if (isDrawerOnMobile && isMobile) {
    return /*#__PURE__*/React__default.createElement(LayerPresentation.LayerPresentation, {
      closeLinkLabel: Localization.texts.lsg.component.Flyout.closeMenu,
      layout: "right-25",
      open: open,
  ...
  let themeClass;
  while (themeElement) {
    const themeClasses = Array.from(themeElement.classList).filter(c => c.startsWith(`${prefix.lsgPreGlobal}theme__`) && !c.startsWith(`${prefix.lsgPreGlobal}theme__bg-`));
    if (themeClasses.length) {
      themeClass = themeClasses[0].replace(`${prefix.lsgPreGlobal}theme__`, "");
      break;
    }
    themeElement = themeElement.closest(`[class*="${prefix.lsgPreGlobal}theme__"]`);
  }
  const content = (open || transitionState !== "exited") && (/*#__PURE__*/React__default.createElement("div", {
    className: DomUtils.cleanupClassObject({
      [ThemePresentation.getThemeClass(themeClass)]: themeClass
    })
  }, /*#__PURE__*/React__default.createElement(Component, {
    id: id,
    className: DomUtils.cleanupClassObject({
      [className]: !!className,
      [Flyout_styles.hostClass]: true,
      [`${Flyout_styles.hostClass}__place-above`]: placeAbove,
      [`${Flyout_styles.hostClass}__spacing-${innerSpacing}`]: innerSpacing,
      [`${Flyout_styles.hostClass}__${transitionState}`]: true,
      [`${prefix.lsgPre}no-margin`]: noMargin,
      [ThemePresentation.getThemeClass("elevated")]: true
    }),
    ref: internalRef,
  ...
  return /*#__PURE__*/React__default.createElement(React__default.Fragment, null, /*#__PURE__*/React__default.createElement("div", {
    ref: inlineRef,
    "aria-hidden": true
  }), /*#__PURE__*/reactDom.createPortal(content, scrollElement));
});
FlyoutPresentation.displayName = "FlyoutPresentation";

exports.FlyoutPresentation = FlyoutPresentation;
