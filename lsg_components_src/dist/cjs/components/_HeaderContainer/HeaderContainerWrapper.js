...
var HeaderContainerPresentation = require('./HeaderContainerPresentation.js');

// @ts-strict-ignore
class HeaderContainerWrapper extends React__default.Component {
  constructor(props) {
    super(props);
    this.mainContainerRightTopRef = /*#__PURE__*/React__default.createRef();
    this.mainContainerRightBottomRef = /*#__PURE__*/React__default.createRef();
    ...
    this.checkMarginLeft(this.mainContainerRightTopRef?.current) && this.checkMarginLeft(this.mainContainerRightBottomRef?.current);
    this.onResize = () => {
    ...
    this.addOffsetClasses = () => {
      ...
      if (this.props.width === "page") {
        ...
        document.body.classList.add(`${prefix.lsgPre}stage__header-anchor-${stageOffset}`);
      }
      // Add classes for scroll padding
      if (this.props.position) {
        const scrollPaddingClass = this.props.isMobile || hasSecondLine ? utilityClassesScroll_styles.utilityClassScrollPadding16 : utilityClassesScroll_styles.utilityClassScrollPadding22;
        const closestScrollElement = this.mainContainerRightTopRef?.current?.closest(`.${utilityClassesScroll_styles.utilityClassScroll}`);
        const scrollElements = closestScrollElement ? [closestScrollElement] : [document.documentElement, document.body];
    ...
    this.removeOffsetClasses = () => {
      document.body.classList.remove(`${prefix.lsgPre}stage__header-anchor-16`);
      document.body.classList.remove(`${prefix.lsgPre}stage__header-anchor-22`);
      document.body.classList.remove(`${prefix.lsgPre}stage__header-anchor-32`);
      ...
      const closestScrollElement = this.mainContainerRightTopRef?.current?.closest(`.${utilityClassesScroll_styles.utilityClassScroll}`);
      const affectedElements = closestScrollElement ? [closestScrollElement] : [document.documentElement, document.body];
      affectedElements.forEach(element => {
        element?.classList.remove(utilityClassesScroll_styles.utilityClassScrollPadding16);
        element?.classList.remove(utilityClassesScroll_styles.utilityClassScrollPadding22);
      });
    };
    this.onScrollChange = () => {
  ...
  componentDidMount() {
    this.addOffsetClasses();
    ScrollEvents.addScrollCallback(this.onScrollChange);
    ResizeEvents.addResizeCallback(this.onResize);
    this.onResize();
  }
  componentWillUnmount() {
    this.removeOffsetClasses();
    ScrollEvents.removeScrollCallback(this.onScrollChange);
    ResizeEvents.removeResizeCallback(this.onResize);
  }
  componentDidUpdate(prevProps) {
    const hadSecondLine = prevProps.bottomRight || prevProps.bottomLeft;
    const hasSecondLine = this.props.bottomRight || this.props.bottomLeft;
    if (prevProps.isMobile !== this.props.isMobile || hadSecondLine !== hasSecondLine || prevProps.position !== this.props.position) {
      this.removeOffsetClasses();
      this.addOffsetClasses();
    }
  ...
  render() {
    ...
    return /*#__PURE__*/React__default.createElement(HeaderContainerPresentation.HeaderContainerPresentation, {
      ...this.props,
      isMobile: forceMobile || this.props.isMobile,
      scrollPosition: (bottomLeft || bottomRight) && position === "fixed" ? scrollPosition : undefined,
      mainContainerRightTopRef: this.mainContainerRightTopRef,
      mainContainerRightBottomRef: this.mainContainerRightBottomRef
    });
  }
}
...
exports.HeaderContainerWrapper = HeaderContainerWrapper;
