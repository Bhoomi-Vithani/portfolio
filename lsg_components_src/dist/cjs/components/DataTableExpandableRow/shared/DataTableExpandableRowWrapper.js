...
const DataTableExpandableRowWrapper = ({
  id,
  children,
  ...
  const getLabel = labelText => (/*#__PURE__*/React__default.createElement(React__default.Fragment, null, /*#__PURE__*/React__default.createElement("span", {
    className: `${DataTable_styles.hostClass}-expandable-label`
  }, labelText), !hideBadge && (/*#__PURE__*/React__default.createElement(BadgePresentation.BadgePresentation, {
    color: badgeColor,
    inline: true,
    margin: "right"
  }, ReactUtils.fragmentCount(children).toString(10))), isInteractive && (/*#__PURE__*/React__default.createElement(IconPresentation.IconPresentation, {
  ...
  const dataProcessed = columnsPreprocessed.reduce((acc, h, i) => {
    ...
    const colspan = isLastValue ? columnsPreprocessed.length - i : Math.max(colspans?.[acc.dataIndex] || 1, acc.remaining);
    ...
    if (h.isLabelColumn) {
      const previousColumns = acc.columns.length > 0 ? [...acc.columns.slice(0, -1), {
  ...
  const childrenIds = ReactUtils.fragmentMap(children, (child, i) => getId(child, id, i))?.join(" ");
  const currentInsetLevel = insetLevel >= 0 ? insetLevel : 0;
  return /*#__PURE__*/React__default.createElement(React__default.Fragment, null, /*#__PURE__*/React__default.createElement(DataTableRowWrapper.DataTableRowWrapper, {
    ...otherProps,
    spacing: spacing,
    ...
    inputType: inputType,
    data: dataProcessed.columns.map(c => c.contents),
    colspans: dataProcessed.columns.map(c => c.colspan),
    ids: [...Array.from(Array(dataProcessed.labelColumn).keys()).map(_ => undefined), `${id}-th`],
    id: id,
    onLinkClick: isInteractive ? () => {
      if (onOpenChange) {
        onOpenChange(!open);
      }
    ...
    htmlLinkAttrs: isInteractive && childrenIds ? {
      "aria-controls": childrenIds,
      "aria-expanded": open
    } : undefined,
    isHeader: false,
    insetLevel: currentInsetLevel
  }), ReactUtils.fragmentMap(children, (child, i) => /*#__PURE__*/React__default.cloneElement(child, {
    id: getId(child, id, i),
    columnsSeparated,
    columnsPreprocessed,
    gridStructure,
    inputType,
    hidden: !open,
    className: i === 0 ? `${child.props.className || ""} ${DataTable_styles.hostClass}-tr__expandable__first-child` : child.props.className,
    ...
    insetLevel: /*#__PURE__*/React__default.isValidElement(child) && child.type?.displayName === "DataTableExpandableRow" ? currentInsetLevel + 1 : undefined
  })));
};

exports.DataTableExpandableRowWrapper = DataTableExpandableRowWrapper;
