'use strict';

var React__default = require('react');
var DomUtils = require('../../utils/DomUtils.js');
var index = require('../../utils/Hooks/index.js');
var Localization = require('../../utils/Localization.js');
var ActionPresentation = require('../Action/shared/ActionPresentation.js');
var Logo_styles = require('./Logo.styles.js');

// @ts-strict-ignore
/* eslint-disable no-nested-ternary */
/* eslint-disable @typescript-eslint/no-use-before-define */
const getRadientStopColors = /*#__PURE__*/React__default.createElement(React__default.Fragment, null, /*#__PURE__*/React__default.createElement("stop", {
  offset: "0",
  stopColor: "#FFEF19"
}), /*#__PURE__*/React__default.createElement("stop", {
  offset: "0.246",
  stopColor: "#FFDF08"
}), /*#__PURE__*/React__default.createElement("stop", {
  offset: "0.44",
  stopColor: "#FFD700"
}), /*#__PURE__*/React__default.createElement("stop", {
  offset: "0.588",
  stopColor: "#FDD401"
}), /*#__PURE__*/React__default.createElement("stop", {
  offset: "0.713",
  stopColor: "#F7C903"
}), /*#__PURE__*/React__default.createElement("stop", {
  offset: "0.829",
  stopColor: "#ECB808"
}), /*#__PURE__*/React__default.createElement("stop", {
  offset: "0.9",
  stopColor: "#E3A90B"
}), /*#__PURE__*/React__default.createElement("stop", {
  offset: "1",
  stopColor: "#D98B07"
}));
const LogoPresentation = props => {
  const {
    id,
    className,
    srcCustom,
    logoLabel,
    isMonochrome = false,
    variant = "mobile",
    fitToBox = false,
    scrollPosition,
    disabled,
    href,
    actionAs,
    actionProps,
    onClick,
    htmlAttrs,
    ...otherProps
  } = props;
  const [{
    hasKeyboardFocus
  }, {
    setHasKeyboardFocus,
    setHasMouseHover
  }, {
    onMouseDown,
    onMouseUp,
    onMouseLeave
  }] = index.useLinkHover(props);
  const wordMarkAnimation = {
    opacity: scrollPosition ? 1 - scrollPosition / 30 : undefined
  };
  const svgId = index.useUniqueId(`${Logo_styles.hostClass}-`, id && `${id}-svg`);
  // If the logo is disabled, or has no href nor an action, we render it as an image without any interaction.
  const disabledResult = disabled || !(href || actionAs || actionProps || onClick);
  // Use the logoLabel prop if provided, else use default texts for Coba- or custom logo.
  // If the logoLabel is " " the image will be a11y hidden (rare decorative cases).
  let labelResult;
  if (!logoLabel) {
    labelResult = srcCustom ? Localization.texts.lsg.component.Logo.title : Localization.texts.lsg.component.Logo.titleCoba;
  } else {
    labelResult = logoLabel.trim();
  }
  /**
   * Check if an external source is defined (srcCustom), otherwise use internal Coba default logo.
   * If the Coba logo is used, the variant and monochrome settings will determine which SVG to render.
   */
  function getLogoElement() {
    return /*#__PURE__*/React__default.createElement(React__default.Fragment, null, srcCustom ? (/*#__PURE__*/React__default.createElement("img", {
      className: `${Logo_styles.hostClass}-custom-logo`,
      src: srcCustom,
      alt: labelResult
    })) : variant === "mobile" ? !isMonochrome ? (/*#__PURE__*/React__default.createElement(LogoSmallSvg, {
      svgId: svgId,
      title: labelResult
    })) : (/*#__PURE__*/React__default.createElement(LogoSmallSvgMonochrome, {
      svgId: svgId,
      title: labelResult
    })) : variant === "vertical" ? !isMonochrome ? (/*#__PURE__*/React__default.createElement(LogoVerticalSvg, {
      svgId: svgId,
      wordMarkAnimation: wordMarkAnimation,
      title: labelResult
    })) : (/*#__PURE__*/React__default.createElement(LogoVerticalSvgMonochrome, {
      svgId: svgId,
      wordMarkAnimation: wordMarkAnimation,
      title: labelResult
    })) : !isMonochrome ? (/*#__PURE__*/React__default.createElement(LogoHorizontalSvg, {
      svgId: svgId,
      title: labelResult
    })) : (/*#__PURE__*/React__default.createElement(LogoHorizontalSvgMonochrome, {
      svgId: svgId,
      title: labelResult
    })));
  }
  return /*#__PURE__*/React__default.createElement("div", {
    id: id,
    className: DomUtils.cleanupClassObject({
      [className]: !!className,
      [Logo_styles.hostClass]: true,
      [`${Logo_styles.hostClass}__monochrome`]: isMonochrome,
      [`${Logo_styles.hostClass}__fit`]: fitToBox
    }),
    "aria-hidden": !labelResult,
    "data-lsg-component": "logo"
  }, disabledResult ? getLogoElement() : (/*#__PURE__*/React__default.createElement(ActionPresentation.ActionPresentation, {
    ...otherProps,
    href: href,
    actionAs: actionAs,
    actionProps: actionProps,
    onClick: onClick,
    htmlAttrs: {
      "aria-label": labelResult,
      ...htmlAttrs
    },
    hasKeyboardFocus: hasKeyboardFocus && !disabled,
    onKeyboardFocusChange: setHasKeyboardFocus,
    onMouseHoverChange: setHasMouseHover,
    onMouseDown: onMouseDown,
    onMouseUp: onMouseUp,
    onMouseLeave: onMouseLeave
  }, getLogoElement())));
};
LogoPresentation.displayName = "LogoPresentation";
// Todo: Please make sure to add id, classes and role to the <svg> element when a new image is provided!
//  The ids should be unique (e.g. id={props.svgId}) also the group (<g>) ids (e.g.: id=`${logoId}Bildmarke_1_`).
//  Also add, if not included, a <title>-Tag to the <svg> and link it to the id for accessibility reasons.
//  To display the logos word-mark (Commerzbank) in a theme specific color a css class is set on the group (<g>) tag
//  to enable the specific styling (see Logo.styles.ts).
const LogoVerticalSvg = props => (/*#__PURE__*/React__default.createElement("svg", {
  id: props.svgId,
  "data-name": "Logo vertical RGB",
  xmlns: "http://www.w3.org/2000/svg",
  width: "120px",
  height: "80px",
  viewBox: "0 0 120 80",
  preserveAspectRatio: "xMinYMin meet",
  className: DomUtils.cleanupClassObject({
    [`${Logo_styles.hostClass}-vertical-svg`]: true,
    [`${Logo_styles.hostClass}-svg`]: true
  }),
  role: "img",
  focusable: "false",
  "aria-labelledby": `${props.svgId}-title`,
  fill: "currentColor"
}, /*#__PURE__*/React__default.createElement("title", {
  id: `${props.svgId}-title`
}, props.title), /*#__PURE__*/React__default.createElement("defs", null, /*#__PURE__*/React__default.createElement("radialGradient", {
  id: `${props.svgId}-gradient`,
  "data-name": "Coba / RGB / Gradient",
  cx: "68.113",
  cy: "-0.071",
  r: "53.406",
  gradientUnits: "userSpaceOnUse"
}, getRadientStopColors), /*#__PURE__*/React__default.createElement("radialGradient", {
  id: `${props.svgId}-gradient-2`,
  "data-name": "Coba / RGB / Gradient",
  cx: "29.667",
  cy: "39.018",
  r: "53.076",
  gradientUnits: "userSpaceOnUse"
}, getRadientStopColors), /*#__PURE__*/React__default.createElement("radialGradient", {
  id: `${props.svgId}-gradient-3`,
  "data-name": "Coba / RGB / Gradient",
  cx: "82.281",
  cy: "52.934",
  r: "53.363",
  gradientUnits: "userSpaceOnUse"
}, getRadientStopColors)), /*#__PURE__*/React__default.createElement("g", {
  id: "Wortmarke",
  style: {
    ...props.wordMarkAnimation
  }
}, /*#__PURE__*/React__default.createElement("path", {
  d: "M5.159,79.99 L5.159,79.99 C2.32,79.99 0.01,77.8 0.01,74.626 C0.01,71.593 2.36,69.164 5.295,69.164 C6.744,69.164 8.016,69.753 8.878,70.692 L7.41,72.218 C6.823,71.652 6.099,71.337 5.256,71.337 C3.651,71.337 2.516,72.768 2.516,74.568 C2.516,76.429 3.632,77.817 5.295,77.817 C6.255,77.817 6.999,77.486 7.586,76.858 L8.995,78.308 C8.036,79.404 6.783,79.99 5.159,79.99 L5.159,79.99 z M15.209,77.857 L15.209,77.857 C17.048,77.857 18.164,76.349 18.164,74.586 C18.164,72.728 17.009,71.298 15.209,71.298 C13.368,71.298 12.252,72.807 12.252,74.568 C12.252,76.429 13.407,77.857 15.209,77.857 L15.209,77.857 z M15.169,79.99 L15.169,79.99 C11.998,79.99 9.786,77.681 9.786,74.626 C9.786,71.612 12.135,69.164 15.247,69.164 C18.419,69.164 20.63,71.475 20.63,74.528 C20.63,77.545 18.281,79.99 15.169,79.99 L15.169,79.99 z M29.317,79.795 L31.079,74.059 C31.392,73.06 31.626,71.926 31.626,71.926 L31.666,71.926 C31.666,71.926 31.686,73.06 31.783,74.197 L32.253,79.795 L34.562,79.795 L33.545,69.342 L30.511,69.342 L28.925,74.528 C28.671,75.372 28.397,76.448 28.397,76.448 C28.397,76.448 28.124,75.372 27.848,74.509 L26.243,69.342 L23.15,69.342 L22.133,79.795 L24.305,79.795 L24.776,74.157 C24.873,73.06 24.893,71.926 24.893,71.926 L24.932,71.926 C24.932,71.926 25.186,73.06 25.499,74.1 L27.261,79.795 L29.317,79.795 L29.317,79.795 z M43.901,79.795 L45.662,74.059 C45.976,73.06 46.211,71.926 46.211,71.926 L46.249,71.926 C46.249,71.926 46.27,73.06 46.367,74.197 L46.837,79.795 L49.147,79.795 L48.128,69.342 L45.094,69.342 L43.51,74.528 C43.254,75.372 42.981,76.448 42.981,76.448 C42.981,76.448 42.706,75.372 42.432,74.509 L40.826,69.342 L37.733,69.342 L36.715,79.795 L38.889,79.795 L39.359,74.157 C39.457,73.06 39.477,71.926 39.477,71.926 L39.515,71.926 C39.515,71.926 39.769,73.06 40.083,74.1 L41.844,79.795 L43.901,79.795 L43.901,79.795 z M51.648,79.795 L58.441,79.795 L58.441,77.8 L53.919,77.8 L53.919,75.508 L57.756,75.508 L57.756,73.55 L53.919,73.55 L53.919,71.337 L58.382,71.337 L58.382,69.342 L51.648,69.342 L51.648,79.795 L51.648,79.795 z M66.263,72.69 L66.263,72.69 C66.263,71.67 65.558,71.298 64.343,71.298 L63.405,71.298 L63.405,74.081 L64.343,74.081 C65.617,74.081 66.263,73.669 66.263,72.69 L66.263,72.69 z M69.062,79.795 L66.361,79.795 L64.129,75.998 L63.405,75.998 L63.405,79.795 L61.134,79.795 L61.134,69.342 L64.266,69.342 C67.417,69.342 68.611,70.575 68.611,72.69 C68.611,74.041 67.848,75.077 66.497,75.586 L69.062,79.795 L69.062,79.795 z M70.403,79.795 L77.684,79.795 L77.684,77.8 L73.26,77.8 L77.587,71.066 L77.587,69.342 L70.502,69.342 L70.502,71.337 L74.728,71.337 L70.403,78.073 L70.403,79.795 L70.403,79.795 z M82.312,75.489 L82.312,77.838 L83.997,77.838 C85.153,77.838 85.661,77.426 85.661,76.624 C85.661,75.801 85.014,75.489 83.997,75.489 L82.312,75.489 L82.312,75.489 z M82.312,71.298 L82.312,73.569 L83.723,73.569 C84.447,73.569 85.073,73.237 85.073,72.396 C85.073,71.534 84.348,71.298 83.626,71.298 L82.312,71.298 L82.312,71.298 z M80.042,79.795 L80.042,69.342 L83.664,69.342 C85.485,69.342 87.483,69.753 87.483,72.024 C87.483,73.18 86.815,74 85.742,74.333 L85.742,74.353 C87.11,74.528 87.972,75.43 87.972,76.78 C87.972,78.954 85.896,79.795 84.037,79.795 L80.042,79.795 L80.042,79.795 z M91.022,79.795 L88.729,79.795 L92.45,69.342 L95.268,69.342 L98.987,79.795 L96.541,79.795 L95.817,77.662 L91.744,77.662 L91.022,79.795 L91.022,79.795 z M95.152,75.705 L94.232,72.924 C93.978,72.161 93.782,71.456 93.782,71.456 C93.782,71.456 93.585,72.161 93.331,72.944 L92.41,75.705 L95.152,75.705 L95.152,75.705 z M100.408,79.795 L102.523,79.795 L102.523,74.823 C102.523,73.884 102.464,72.924 102.464,72.924 C102.464,72.924 102.953,73.765 103.5,74.586 L107.043,79.795 L109.059,79.795 L109.059,69.342 L106.944,69.342 L106.944,74.081 C106.944,75.017 107.005,75.979 107.005,75.979 C107.005,75.979 106.515,75.137 105.967,74.313 L102.581,69.342 L100.408,69.342 L100.408,79.795 L100.408,79.795 z M111.573,69.342 L111.573,79.795 L113.844,79.795 L113.844,74.901 L117.191,79.795 L119.99,79.795 L116.155,74.197 L119.619,69.342 L116.995,69.342 L113.844,74.041 L113.844,69.342 L111.573,69.342 L111.573,69.342 z",
  id: "Commerzbank",
  className: `${Logo_styles.hostClass}-wordmark`
})), /*#__PURE__*/React__default.createElement("g", {
  id: "Bildmarke"
}, /*#__PURE__*/React__default.createElement("path", {
  d: "M69.584,-0 L68.97,-0 L67.679,-0 L67.679,-0 L43.955,-0 C43.934,-0 43.915,0.012 43.903,0.03 L25.508,31.894 C25.498,31.913 25.498,31.936 25.508,31.954 L38.584,54.607 C38.535,54.45 38.447,54.276 38.447,54.276 C38.171,53.762 37.966,53.364 37.857,53.113 C37.321,51.888 37.009,50.195 37.096,48.546 C37.312,44.202 39.483,39.919 40.641,37.912 C43.368,33.196 48.632,24.078 48.632,24.078 C48.632,24.078 53.896,14.959 56.615,10.243 C57.289,9.075 58.438,7.186 59.999,5.346 L60.048,5.403 C60.048,5.403 60.105,5.34 60.108,5.337 C61.229,4.016 62.563,2.72 64.083,1.737 C65.474,0.839 67.097,0.26 68.429,0.113 C68.869,0.064 69.574,-0 69.574,-0",
  fill: `url(#${props.svgId}-gradient)`
}), /*#__PURE__*/React__default.createElement("path", {
  d: "M91.814,36.546 C91.202,37.573 90.725,38.337 90.467,38.692 C89.677,39.768 88.362,40.889 86.894,41.641 C83.02,43.626 78.16,43.764 75.839,43.764 C70.392,43.764 59.998,43.764 59.998,43.764 C59.998,43.764 49.471,43.764 44.021,43.764 C41.705,43.764 36.975,43.626 33.106,41.641 C31.638,40.889 30.321,39.768 29.531,38.692 C29.27,38.337 28.935,37.891 28.935,37.891 L41.68,59.97 C41.691,59.988 41.71,60 41.733,60 L78.259,60 C78.281,60 78.3,59.988 78.311,59.97 L91.888,36.462 C91.888,36.462 91.859,36.468 91.814,36.546 z",
  fill: `url(#${props.svgId}-gradient-2)`
}), /*#__PURE__*/React__default.createElement("path", {
  d: "M76.04,-0 L69.472,-0 C69.472,-0 68.767,0.064 68.327,0.113 C66.996,0.26 65.373,0.839 63.981,1.737 C62.458,2.722 61.121,4.022 59.998,5.346 C61.558,7.186 62.71,9.074 63.378,10.243 C66.1,14.959 71.37,24.079 71.37,24.079 C71.37,24.079 76.63,33.196 79.355,37.912 C80.514,39.919 82.688,44.202 82.903,48.546 C82.977,50.043 82.73,51.574 82.284,52.761 L81.897,53.761 L94.492,31.952 C94.503,31.934 94.503,31.911 94.492,31.892 L76.092,0.03 C76.081,0.012 76.062,-0 76.04,-0 z",
  fill: `url(#${props.svgId}-gradient-3)`
}))));
const LogoHorizontalSvg = props => (/*#__PURE__*/React__default.createElement("svg", {
  id: props.svgId,
  "data-name": "Logo horizontal RGB",
  xmlns: "http://www.w3.org/2000/svg",
  x: "0",
  y: "0",
  width: "300",
  height: "40",
  viewBox: "0, 0, 300, 40",
  preserveAspectRatio: "xMinYMin meet",
  className: DomUtils.cleanupClassObject({
    [`${Logo_styles.hostClass}-horizontal-svg`]: true,
    [`${Logo_styles.hostClass}-svg`]: true
  }),
  role: "img",
  focusable: "false",
  "aria-labelledby": `${props.svgId}-title`,
  fill: "currentColor"
}, /*#__PURE__*/React__default.createElement("title", {
  id: `${props.svgId}-title`
}, props.title), /*#__PURE__*/React__default.createElement("defs", null, /*#__PURE__*/React__default.createElement("radialGradient", {
  id: `${props.svgId}-gradient`,
  "data-name": "Coba / RGB / Gradient",
  gradientUnits: "userSpaceOnUse",
  cx: "40.753",
  cy: "-212.504",
  r: "53.403",
  gradientTransform: "matrix(0.667, 0, 0, 0.667, 255.229, 141.629)"
}, getRadientStopColors), /*#__PURE__*/React__default.createElement("radialGradient", {
  id: `${props.svgId}-gradient-2`,
  "data-name": "Coba / RGB / Gradient",
  gradientUnits: "userSpaceOnUse",
  cx: "2.31",
  cy: "-173.416",
  r: "53.072",
  gradientTransform: "matrix(0.667, 0, 0, 0.667, 255.229, 141.629)"
}, getRadientStopColors), /*#__PURE__*/React__default.createElement("radialGradient", {
  id: `${props.svgId}-gradient-3`,
  "data-name": "Coba / RGB / Gradient",
  gradientUnits: "userSpaceOnUse",
  cx: "54.921",
  cy: "-159.501",
  r: "53.36",
  gradientTransform: "matrix(0.667, 0, 0, 0.667, 255.229, 141.629)"
}, getRadientStopColors)), /*#__PURE__*/React__default.createElement("g", {
  id: "Wortmarke"
}, /*#__PURE__*/React__default.createElement("path", {
  d: "M10.474,33.059 L10.474,33.059 C4.704,33.059 0.01,28.608 0.01,22.159 C0.01,15.993 4.785,11.056 10.751,11.056 C13.697,11.056 16.282,12.254 18.032,14.162 L15.05,17.264 C13.856,16.112 12.385,15.472 10.672,15.472 C7.41,15.472 5.104,18.381 5.104,22.04 C5.104,25.822 7.371,28.642 10.751,28.642 C12.702,28.642 14.214,27.971 15.407,26.694 L18.271,29.64 C16.322,31.87 13.776,33.059 10.474,33.059 L10.474,33.059 z M30.9,28.725 L30.9,28.725 C34.638,28.725 36.907,25.66 36.907,22.076 C36.907,18.301 34.559,15.394 30.9,15.394 C27.16,15.394 24.891,18.461 24.891,22.04 C24.891,25.822 27.239,28.725 30.9,28.725 L30.9,28.725 z M30.818,33.059 L30.818,33.059 C24.375,33.059 19.879,28.367 19.879,22.159 C19.879,16.032 24.652,11.056 30.979,11.056 C37.425,11.056 41.919,15.754 41.919,21.959 C41.919,28.091 37.145,33.059 30.818,33.059 L30.818,33.059 z M59.573,32.663 L63.154,21.005 C63.79,18.975 64.267,16.669 64.267,16.669 L64.348,16.669 C64.348,16.669 64.388,18.975 64.587,21.285 L65.542,32.663 L70.234,32.663 L68.168,11.419 L62.001,11.419 L58.777,21.959 C58.261,23.673 57.704,25.861 57.704,25.861 C57.704,25.861 57.149,23.673 56.589,21.92 L53.326,11.419 L47.04,11.419 L44.972,32.663 L49.388,32.663 L50.345,21.205 C50.542,18.975 50.582,16.669 50.582,16.669 L50.661,16.669 C50.661,16.669 51.179,18.975 51.814,21.087 L55.396,32.663 L59.573,32.663 L59.573,32.663 z M89.213,32.663 L92.793,21.005 C93.43,18.975 93.909,16.669 93.909,16.669 L93.987,16.669 C93.987,16.669 94.028,18.975 94.226,21.285 L95.18,32.663 L99.876,32.663 L97.806,11.419 L91.64,11.419 L88.419,21.959 C87.9,23.673 87.344,25.861 87.344,25.861 C87.344,25.861 86.785,23.673 86.229,21.92 L82.966,11.419 L76.679,11.419 L74.611,32.663 L79.027,32.663 L79.983,21.205 C80.182,18.975 80.222,16.669 80.222,16.669 L80.301,16.669 C80.301,16.669 80.817,18.975 81.454,21.087 L85.034,32.663 L89.213,32.663 L89.213,32.663 z M104.96,32.663 L118.766,32.663 L118.766,28.608 L109.575,28.608 L109.575,23.951 L117.373,23.951 L117.373,19.971 L109.575,19.971 L109.575,15.472 L118.645,15.472 L118.645,11.419 L104.96,11.419 L104.96,32.663 L104.96,32.663 z M134.662,18.223 L134.662,18.223 C134.662,16.151 133.23,15.394 130.761,15.394 L128.854,15.394 L128.854,21.049 L130.761,21.049 C133.35,21.049 134.662,20.212 134.662,18.223 L134.662,18.223 z M140.351,32.663 L134.862,32.663 L130.325,24.946 L128.855,24.946 L128.855,32.663 L124.239,32.663 L124.239,11.419 L130.604,11.419 C137.009,11.419 139.436,13.924 139.436,18.223 C139.436,20.968 137.885,23.074 135.139,24.109 L140.351,32.663 L140.351,32.663 z M143.077,32.663 L157.875,32.663 L157.875,28.608 L148.884,28.608 L157.678,14.922 L157.678,11.419 L143.277,11.419 L143.277,15.472 L151.867,15.472 L143.077,29.162 L143.077,32.663 L143.077,32.663 z M167.281,23.912 L167.281,28.685 L170.706,28.685 C173.055,28.685 174.088,27.849 174.088,26.219 C174.088,24.547 172.773,23.912 170.706,23.912 L167.281,23.912 L167.281,23.912 z M167.281,15.394 L167.281,20.01 L170.149,20.01 C171.622,20.01 172.892,19.335 172.892,17.626 C172.892,15.874 171.42,15.394 169.952,15.394 L167.281,15.394 L167.281,15.394 z M162.669,32.663 L162.669,11.419 L170.029,11.419 C173.731,11.419 177.791,12.254 177.791,16.87 C177.791,19.218 176.434,20.886 174.251,21.563 L174.251,21.602 C177.033,21.959 178.784,23.792 178.784,26.536 C178.784,30.954 174.565,32.663 170.787,32.663 L162.669,32.663 L162.669,32.663 z M184.984,32.663 L180.324,32.663 L187.886,11.419 L193.613,11.419 L201.171,32.663 L196.201,32.663 L194.728,28.328 L186.451,28.328 L184.984,32.663 L184.984,32.663 z M193.377,24.35 L191.507,18.699 C190.992,17.147 190.592,15.715 190.592,15.715 C190.592,15.715 190.193,17.147 189.677,18.739 L187.805,24.35 L193.377,24.35 L193.377,24.35 z M204.059,32.663 L208.357,32.663 L208.357,22.558 C208.357,20.65 208.238,18.699 208.238,18.699 C208.238,18.699 209.233,20.408 210.344,22.076 L217.544,32.663 L221.642,32.663 L221.642,11.419 L217.345,11.419 L217.345,21.049 C217.345,22.952 217.469,24.907 217.469,24.907 C217.469,24.907 216.471,23.196 215.358,21.521 L208.477,11.419 L204.059,11.419 L204.059,32.663 L204.059,32.663 z M226.75,11.419 L226.75,32.663 L231.366,32.663 L231.366,22.717 L238.17,32.663 L243.859,32.663 L236.064,21.285 L243.105,11.419 L237.771,11.419 L231.366,20.968 L231.366,11.419 L226.75,11.419 L226.75,11.419 z",
  id: "Commerzbank",
  className: `${Logo_styles.hostClass}-wordmark`
})), /*#__PURE__*/React__default.createElement("g", {
  id: "Bildmarke"
}, /*#__PURE__*/React__default.createElement("path", {
  d: "M283.38,0 L282.97,0 L282.11,0 L282.11,0 L266.293,0 C266.28,0 266.267,0.008 266.259,0.02 L253.995,21.263 C253.989,21.275 253.989,21.29 253.995,21.303 L262.713,36.405 C262.68,36.3 262.622,36.184 262.622,36.184 C262.438,35.841 262.301,35.576 262.228,35.409 C261.871,34.591 261.663,33.464 261.721,32.364 C261.865,29.468 263.312,26.612 264.084,25.275 C265.902,22.131 269.411,16.052 269.411,16.052 C269.411,16.052 272.92,9.972 274.733,6.828 C275.183,6.05 275.949,4.79 276.989,3.564 L277.022,3.602 C277.022,3.602 277.06,3.56 277.062,3.558 C277.809,2.677 278.699,1.813 279.712,1.158 C280.639,0.559 281.722,0.173 282.609,0.075 C282.903,0.043 283.373,0 283.373,0",
  fill: `url(#${props.svgId}-gradient)`
}), /*#__PURE__*/React__default.createElement("path", {
  d: "M298.199,24.364 C297.791,25.049 297.473,25.558 297.301,25.795 C296.775,26.512 295.898,27.259 294.919,27.761 C292.337,29.084 289.097,29.176 287.55,29.176 C283.918,29.176 276.989,29.176 276.989,29.176 C276.989,29.176 269.971,29.176 266.338,29.176 C264.793,29.176 261.641,29.084 259.061,27.761 C258.082,27.259 257.204,26.512 256.678,25.795 C256.504,25.558 256.28,25.261 256.28,25.261 L264.777,39.98 C264.785,39.992 264.797,40 264.812,40 L289.163,40 C289.177,40 289.19,39.992 289.197,39.98 L298.249,24.308 C298.248,24.308 298.23,24.312 298.199,24.364 z",
  fill: `url(#${props.svgId}-gradient-2)`
}), /*#__PURE__*/React__default.createElement("path", {
  d: "M287.683,-0 L283.305,-0 C283.305,-0 282.835,0.043 282.542,0.075 C281.654,0.173 280.572,0.56 279.645,1.158 C278.629,1.815 277.737,2.682 276.989,3.564 C278.029,4.79 278.797,6.05 279.242,6.829 C281.057,9.973 284.57,16.052 284.57,16.052 C284.57,16.052 288.077,22.131 289.894,25.275 C290.666,26.612 292.116,29.468 292.258,32.364 C292.308,33.362 292.143,34.383 291.846,35.174 L291.588,35.841 L299.985,21.301 C299.992,21.289 299.992,21.274 299.985,21.261 L287.718,0.02 C287.711,0.008 287.698,-0 287.683,-0 z",
  fill: `url(#${props.svgId}-gradient-3)`
}))));
const LogoSmallSvg = props => (/*#__PURE__*/React__default.createElement("svg", {
  id: props.svgId,
  "data-name": "Logo small RGB",
  xmlns: "http://www.w3.org/2000/svg",
  x: "0",
  y: "0",
  width: "116",
  height: "100",
  viewBox: "0, 0, 116, 100",
  preserveAspectRatio: "xMinYMin meet",
  className: DomUtils.cleanupClassObject({
    [`${Logo_styles.hostClass}-small-svg`]: true,
    [`${Logo_styles.hostClass}-svg`]: true
  }),
  role: "img",
  focusable: "false",
  "aria-labelledby": `${props.svgId}-title`
}, /*#__PURE__*/React__default.createElement("title", {
  id: `${props.svgId}-title`
}, props.title), /*#__PURE__*/React__default.createElement("defs", null, /*#__PURE__*/React__default.createElement("radialGradient", {
  id: `${props.svgId}-gradient`,
  "data-name": "Coba / RGB / Gradient",
  cx: "71.521",
  cy: "-0.119",
  r: "89.01",
  gradientUnits: "userSpaceOnUse"
}, getRadientStopColors), /*#__PURE__*/React__default.createElement("radialGradient", {
  id: `${props.svgId}-gradient-2`,
  "data-name": "Coba / RGB / Gradient",
  cx: "7.446",
  cy: "65.031",
  r: "88.459",
  gradientUnits: "userSpaceOnUse"
}, getRadientStopColors), /*#__PURE__*/React__default.createElement("radialGradient", {
  id: `${props.svgId}-gradient-3`,
  "data-name": "Coba / RGB / Gradient",
  cx: "95.135",
  cy: "88.224",
  r: "88.939",
  gradientUnits: "userSpaceOnUse"
}, getRadientStopColors)), /*#__PURE__*/React__default.createElement("g", {
  id: "Bildmarke"
}, /*#__PURE__*/React__default.createElement("path", {
  d: "M73.974,0 L72.95,0 L70.799,0 L70.799,0 L31.259,0 C31.223,0 31.191,0.019 31.173,0.05 L0.513,53.157 C0.496,53.188 0.496,53.226 0.513,53.257 L22.308,91.012 C22.225,90.75 22.078,90.459 22.078,90.459 C21.619,89.604 21.277,88.94 21.094,88.522 C20.202,86.479 19.682,83.659 19.826,80.91 C20.187,73.671 23.806,66.531 25.734,63.187 C30.28,55.326 39.053,40.131 39.053,40.131 C39.053,40.131 47.826,24.931 52.358,17.071 C53.482,15.125 55.396,11.977 57.997,8.91 L58.08,9.004 C58.08,9.004 58.175,8.9 58.18,8.895 C60.048,6.694 62.272,4.533 64.805,2.895 C67.124,1.398 69.829,0.433 72.048,0.189 C72.781,0.107 73.956,0 73.956,0",
  fill: `url(#${props.svgId}-gradient)`
}), /*#__PURE__*/React__default.createElement("path", {
  d: "M111.023,60.911 C110.003,62.622 109.208,63.894 108.778,64.487 C107.462,66.281 105.271,68.148 102.824,69.402 C96.366,72.709 88.266,72.94 84.398,72.94 C75.319,72.94 57.997,72.94 57.997,72.94 C57.997,72.94 40.451,72.94 31.369,72.94 C27.508,72.94 19.625,72.709 13.177,69.402 C10.729,68.148 8.535,66.281 7.218,64.487 C6.784,63.894 6.225,63.152 6.225,63.152 L27.467,99.949 C27.485,99.98 27.518,100 27.554,100 L88.432,100 C88.468,100 88.5,99.98 88.518,99.949 L111.146,60.769 C111.146,60.769 111.099,60.781 111.023,60.911 z",
  fill: `url(#${props.svgId}-gradient-2)`
}), /*#__PURE__*/React__default.createElement("path", {
  d: "M84.733,-0 L73.787,-0 C73.787,-0 72.612,0.107 71.879,0.189 C69.66,0.433 66.955,1.398 64.635,2.895 C62.096,4.537 59.868,6.704 57.997,8.91 C60.597,11.977 62.516,15.124 63.63,17.071 C68.168,24.931 76.951,40.131 76.951,40.131 C76.951,40.131 85.717,55.326 90.258,63.187 C92.189,66.531 95.814,73.671 96.171,80.91 C96.295,83.405 95.882,85.957 95.14,87.935 L94.494,89.602 L115.486,53.254 C115.504,53.223 115.504,53.185 115.486,53.154 L84.82,0.05 C84.802,0.019 84.769,-0 84.733,-0 z",
  fill: `url(#${props.svgId}-gradient-3)`
}))));
const LogoSmallSvgMonochrome = props => (/*#__PURE__*/React__default.createElement("svg", {
  id: props.svgId,
  "data-name": "Logo small monochrome",
  xmlns: "http://www.w3.org/2000/svg",
  x: "0",
  y: "0",
  width: "116",
  height: "100",
  viewBox: "0, 0, 116, 100",
  preserveAspectRatio: "xMinYMin meet",
  className: DomUtils.cleanupClassObject({
    [`${Logo_styles.hostClass}-monochrome-small-svg`]: true,
    [`${Logo_styles.hostClass}-svg`]: true
  }),
  role: "img",
  focusable: "false",
  "aria-labelledby": `${props.svgId}-title`,
  fill: "currentColor"
}, /*#__PURE__*/React__default.createElement("title", {
  id: `${props.svgId}-title`
}, props.title), /*#__PURE__*/React__default.createElement("path", {
  id: "Bildmarke",
  d: "M90.517,77.468 C90.471,77.092 90.444,76.853 90.404,76.5 C90.344,75.975 90.26,75.41 90.206,75.015 C90.135,74.505 89.972,73.572 89.856,73.018 C89.846,72.974 89.804,72.944 89.758,72.944 L58.008,72.944 C58.008,72.944 32.17,72.942 28.716,72.94 C28.473,72.94 28.25,72.938 28.207,72.938 C20.691,72.798 13.954,70.35 8.243,66.271 C8.156,66.209 8.046,66.31 8.099,66.403 L27.464,99.95 C27.482,99.981 27.515,100 27.55,100 L84.665,100 C84.697,100 84.728,99.984 84.746,99.958 C87.006,96.83 88.66,93.347 89.666,89.592 C90.606,86.086 90.938,82.502 90.659,78.918 C90.621,78.434 90.577,77.951 90.517,77.468 z M86.373,2.743 C86.356,2.715 86.326,2.696 86.293,2.693 C85.446,2.624 84.595,2.59 83.743,2.59 L83.742,2.59 C78.268,2.591 73.08,4.014 68.312,6.707 C65.902,8.067 63.682,9.739 61.714,11.675 C61.478,11.907 61.245,12.143 61.017,12.383 C60.986,12.415 60.981,12.463 61.003,12.502 C62.112,14.425 87.566,58.561 91.337,65.131 C97.489,75.848 94.807,87.067 93.941,90.161 C93.91,90.275 94.065,90.339 94.124,90.238 L115.484,53.254 C115.501,53.223 115.501,53.185 115.484,53.154 L86.373,2.743 z M31.169,0.05 L1.212,51.942 C1.194,51.974 1.195,52.014 1.215,52.045 C1.562,52.588 4.621,57.331 6.704,59.414 C9.615,62.325 13.004,64.61 16.777,66.206 C18.801,67.062 20.667,67.699 22.806,68.115 C22.848,68.123 22.891,68.104 22.912,68.067 C24.059,66.08 52.717,16.465 52.803,16.319 C57.376,8.397 64.509,2.876 72.555,0.194 C72.663,0.158 72.637,-0 72.524,-0 L31.256,-0 C31.22,-0 31.187,0.02 31.169,0.05 z"
})));
const LogoVerticalSvgMonochrome = props => (/*#__PURE__*/React__default.createElement("svg", {
  id: props.svgId,
  "data-name": "Logo vertical monochrome",
  xmlns: "http://www.w3.org/2000/svg",
  width: "120",
  height: "80",
  viewBox: "0 0 120 80",
  preserveAspectRatio: "xMinYMin meet",
  className: DomUtils.cleanupClassObject({
    [`${Logo_styles.hostClass}-monochrome-vertical-svg`]: true,
    [`${Logo_styles.hostClass}-svg`]: true
  }),
  role: "img",
  focusable: "false",
  "aria-labelledby": `${props.svgId}-title`,
  fill: "currentColor"
}, /*#__PURE__*/React__default.createElement("title", {
  id: `${props.svgId}-title`
}, props.title), /*#__PURE__*/React__default.createElement("g", {
  id: "Wortmarke",
  style: {
    ...props.wordMarkAnimation
  }
}, /*#__PURE__*/React__default.createElement("path", {
  id: "Commerzbank",
  d: "M5.159,79.99 L5.159,79.99 C2.32,79.99 0.01,77.8 0.01,74.626 C0.01,71.593 2.36,69.164 5.295,69.164 C6.744,69.164 8.016,69.753 8.878,70.692 L7.41,72.218 C6.823,71.652 6.099,71.337 5.256,71.337 C3.651,71.337 2.516,72.768 2.516,74.568 C2.516,76.429 3.632,77.817 5.295,77.817 C6.255,77.817 6.999,77.486 7.586,76.858 L8.995,78.308 C8.036,79.404 6.783,79.99 5.159,79.99 L5.159,79.99 z M15.209,77.857 L15.209,77.857 C17.048,77.857 18.164,76.349 18.164,74.586 C18.164,72.728 17.009,71.298 15.209,71.298 C13.368,71.298 12.252,72.807 12.252,74.568 C12.252,76.429 13.407,77.857 15.209,77.857 L15.209,77.857 z M15.169,79.99 L15.169,79.99 C11.998,79.99 9.786,77.681 9.786,74.626 C9.786,71.612 12.135,69.164 15.247,69.164 C18.419,69.164 20.63,71.475 20.63,74.528 C20.63,77.545 18.281,79.99 15.169,79.99 L15.169,79.99 z M29.317,79.795 L31.079,74.059 C31.392,73.06 31.626,71.926 31.626,71.926 L31.666,71.926 C31.666,71.926 31.686,73.06 31.783,74.197 L32.253,79.795 L34.562,79.795 L33.545,69.342 L30.511,69.342 L28.925,74.528 C28.671,75.372 28.397,76.448 28.397,76.448 C28.397,76.448 28.124,75.372 27.848,74.509 L26.243,69.342 L23.15,69.342 L22.133,79.795 L24.305,79.795 L24.776,74.157 C24.873,73.06 24.893,71.926 24.893,71.926 L24.932,71.926 C24.932,71.926 25.186,73.06 25.499,74.1 L27.261,79.795 L29.317,79.795 L29.317,79.795 z M43.901,79.795 L45.662,74.059 C45.976,73.06 46.211,71.926 46.211,71.926 L46.249,71.926 C46.249,71.926 46.27,73.06 46.367,74.197 L46.837,79.795 L49.147,79.795 L48.128,69.342 L45.094,69.342 L43.51,74.528 C43.254,75.372 42.981,76.448 42.981,76.448 C42.981,76.448 42.706,75.372 42.432,74.509 L40.826,69.342 L37.733,69.342 L36.715,79.795 L38.889,79.795 L39.359,74.157 C39.457,73.06 39.477,71.926 39.477,71.926 L39.515,71.926 C39.515,71.926 39.769,73.06 40.083,74.1 L41.844,79.795 L43.901,79.795 L43.901,79.795 z M51.648,79.795 L58.441,79.795 L58.441,77.8 L53.919,77.8 L53.919,75.508 L57.756,75.508 L57.756,73.55 L53.919,73.55 L53.919,71.337 L58.382,71.337 L58.382,69.342 L51.648,69.342 L51.648,79.795 L51.648,79.795 z M66.263,72.69 L66.263,72.69 C66.263,71.67 65.558,71.298 64.343,71.298 L63.405,71.298 L63.405,74.081 L64.343,74.081 C65.617,74.081 66.263,73.669 66.263,72.69 L66.263,72.69 z M69.062,79.795 L66.361,79.795 L64.129,75.998 L63.405,75.998 L63.405,79.795 L61.134,79.795 L61.134,69.342 L64.266,69.342 C67.417,69.342 68.611,70.575 68.611,72.69 C68.611,74.041 67.848,75.077 66.497,75.586 L69.062,79.795 L69.062,79.795 z M70.403,79.795 L77.684,79.795 L77.684,77.8 L73.26,77.8 L77.587,71.066 L77.587,69.342 L70.502,69.342 L70.502,71.337 L74.728,71.337 L70.403,78.073 L70.403,79.795 L70.403,79.795 z M82.312,75.489 L82.312,77.838 L83.997,77.838 C85.153,77.838 85.661,77.426 85.661,76.624 C85.661,75.801 85.014,75.489 83.997,75.489 L82.312,75.489 L82.312,75.489 z M82.312,71.298 L82.312,73.569 L83.723,73.569 C84.447,73.569 85.073,73.237 85.073,72.396 C85.073,71.534 84.348,71.298 83.626,71.298 L82.312,71.298 L82.312,71.298 z M80.042,79.795 L80.042,69.342 L83.664,69.342 C85.485,69.342 87.483,69.753 87.483,72.024 C87.483,73.18 86.815,74 85.742,74.333 L85.742,74.353 C87.11,74.528 87.972,75.43 87.972,76.78 C87.972,78.954 85.896,79.795 84.037,79.795 L80.042,79.795 L80.042,79.795 z M91.022,79.795 L88.729,79.795 L92.45,69.342 L95.268,69.342 L98.987,79.795 L96.541,79.795 L95.817,77.662 L91.744,77.662 L91.022,79.795 L91.022,79.795 z M95.152,75.705 L94.232,72.924 C93.978,72.161 93.782,71.456 93.782,71.456 C93.782,71.456 93.585,72.161 93.331,72.944 L92.41,75.705 L95.152,75.705 L95.152,75.705 z M100.408,79.795 L102.523,79.795 L102.523,74.823 C102.523,73.884 102.464,72.924 102.464,72.924 C102.464,72.924 102.953,73.765 103.5,74.586 L107.043,79.795 L109.059,79.795 L109.059,69.342 L106.944,69.342 L106.944,74.081 C106.944,75.017 107.005,75.979 107.005,75.979 C107.005,75.979 106.515,75.137 105.967,74.313 L102.581,69.342 L100.408,69.342 L100.408,79.795 L100.408,79.795 z M111.573,69.342 L111.573,79.795 L113.844,79.795 L113.844,74.901 L117.191,79.795 L119.99,79.795 L116.155,74.197 L119.619,69.342 L116.995,69.342 L113.844,74.041 L113.844,69.342 L111.573,69.342 L111.573,69.342 z"
})), /*#__PURE__*/React__default.createElement("g", {
  id: "Bildmarke"
}, /*#__PURE__*/React__default.createElement("path", {
  id: "Bildmarke",
  d: "M79.51,46.481 C79.483,46.255 79.467,46.112 79.443,45.9 C79.407,45.585 79.356,45.246 79.324,45.01 C79.281,44.703 79.184,44.143 79.114,43.811 C79.108,43.784 79.083,43.766 79.055,43.766 L60.005,43.766 C60.005,43.766 44.502,43.765 42.43,43.764 C42.284,43.764 42.15,43.763 42.124,43.763 C37.615,43.679 33.573,42.21 30.146,39.763 C30.094,39.725 30.028,39.786 30.06,39.842 L41.678,59.97 C41.689,59.989 41.709,60 41.731,60 L75.999,60 C76.019,60 76.037,59.991 76.048,59.975 C77.404,58.098 78.397,56.008 79,53.755 C79.564,51.652 79.763,49.502 79.596,47.351 C79.573,47.061 79.546,46.771 79.51,46.481 z M77.024,1.646 C77.014,1.629 76.996,1.618 76.976,1.616 C76.468,1.574 75.957,1.554 75.446,1.554 L75.446,1.554 C72.161,1.554 69.048,2.408 66.188,4.024 C64.741,4.841 63.409,5.844 62.229,7.005 C62.087,7.144 61.947,7.286 61.811,7.43 C61.792,7.449 61.789,7.478 61.802,7.501 C62.467,8.655 77.74,35.137 80.003,39.079 C83.694,45.509 82.084,52.241 81.565,54.097 C81.546,54.165 81.64,54.204 81.675,54.143 L94.49,31.952 C94.501,31.934 94.501,31.911 94.49,31.892 L77.024,1.646 z M43.902,0.03 L25.927,31.165 C25.916,31.185 25.917,31.208 25.929,31.227 C26.138,31.553 27.973,34.399 29.223,35.649 C30.969,37.396 33.003,38.766 35.266,39.724 C36.48,40.238 37.6,40.62 38.884,40.869 C38.909,40.874 38.934,40.863 38.947,40.84 C39.636,39.648 56.831,9.879 56.882,9.791 C59.626,5.038 63.906,1.725 68.734,0.117 C68.798,0.095 68.782,0 68.715,0 L43.954,0 C43.932,0 43.913,0.012 43.902,0.03 z"
}))));
const LogoHorizontalSvgMonochrome = props => (/*#__PURE__*/React__default.createElement("svg", {
  id: props.svgId,
  "data-name": "Logo horizontal monochrome",
  xmlns: "http://www.w3.org/2000/svg",
  width: "300",
  height: "40",
  viewBox: "0 0 300 40",
  preserveAspectRatio: "xMinYMin meet",
  className: DomUtils.cleanupClassObject({
    [`${Logo_styles.hostClass}-monochrome-horizontal-svg`]: true,
    [`${Logo_styles.hostClass}-svg`]: true
  }),
  role: "img",
  focusable: "false",
  "aria-labelledby": `${props.svgId}-title`,
  fill: "currentColor"
}, /*#__PURE__*/React__default.createElement("title", {
  id: `${props.svgId}-title`
}, props.title), /*#__PURE__*/React__default.createElement("g", {
  id: "Wortmarke"
}, /*#__PURE__*/React__default.createElement("path", {
  id: "Commerzbank",
  d: "M10.474,33.059 L10.474,33.059 C4.704,33.059 0.01,28.608 0.01,22.159 C0.01,15.993 4.785,11.056 10.751,11.056 C13.697,11.056 16.282,12.254 18.032,14.162 L15.05,17.264 C13.856,16.112 12.385,15.472 10.672,15.472 C7.41,15.472 5.104,18.381 5.104,22.04 C5.104,25.822 7.371,28.642 10.751,28.642 C12.702,28.642 14.214,27.971 15.407,26.694 L18.271,29.64 C16.322,31.87 13.776,33.059 10.474,33.059 L10.474,33.059 z M30.9,28.725 L30.9,28.725 C34.638,28.725 36.907,25.66 36.907,22.076 C36.907,18.301 34.559,15.394 30.9,15.394 C27.16,15.394 24.891,18.461 24.891,22.04 C24.891,25.822 27.239,28.725 30.9,28.725 L30.9,28.725 z M30.818,33.059 L30.818,33.059 C24.375,33.059 19.879,28.367 19.879,22.159 C19.879,16.032 24.652,11.056 30.979,11.056 C37.425,11.056 41.919,15.754 41.919,21.959 C41.919,28.091 37.145,33.059 30.818,33.059 L30.818,33.059 z M59.573,32.663 L63.154,21.005 C63.79,18.975 64.267,16.669 64.267,16.669 L64.348,16.669 C64.348,16.669 64.388,18.975 64.587,21.285 L65.542,32.663 L70.234,32.663 L68.168,11.419 L62.001,11.419 L58.777,21.959 C58.261,23.673 57.704,25.861 57.704,25.861 C57.704,25.861 57.149,23.673 56.589,21.92 L53.326,11.419 L47.04,11.419 L44.972,32.663 L49.388,32.663 L50.345,21.205 C50.542,18.975 50.582,16.669 50.582,16.669 L50.661,16.669 C50.661,16.669 51.179,18.975 51.814,21.087 L55.396,32.663 L59.573,32.663 L59.573,32.663 z M89.213,32.663 L92.793,21.005 C93.43,18.975 93.909,16.669 93.909,16.669 L93.987,16.669 C93.987,16.669 94.028,18.975 94.226,21.285 L95.18,32.663 L99.876,32.663 L97.806,11.419 L91.64,11.419 L88.419,21.959 C87.9,23.673 87.344,25.861 87.344,25.861 C87.344,25.861 86.785,23.673 86.229,21.92 L82.966,11.419 L76.679,11.419 L74.611,32.663 L79.027,32.663 L79.983,21.205 C80.182,18.975 80.222,16.669 80.222,16.669 L80.301,16.669 C80.301,16.669 80.817,18.975 81.454,21.087 L85.034,32.663 L89.213,32.663 L89.213,32.663 z M104.96,32.663 L118.766,32.663 L118.766,28.608 L109.575,28.608 L109.575,23.951 L117.373,23.951 L117.373,19.971 L109.575,19.971 L109.575,15.472 L118.645,15.472 L118.645,11.419 L104.96,11.419 L104.96,32.663 L104.96,32.663 z M134.662,18.223 L134.662,18.223 C134.662,16.151 133.23,15.394 130.761,15.394 L128.854,15.394 L128.854,21.049 L130.761,21.049 C133.35,21.049 134.662,20.212 134.662,18.223 L134.662,18.223 z M140.351,32.663 L134.862,32.663 L130.325,24.946 L128.855,24.946 L128.855,32.663 L124.239,32.663 L124.239,11.419 L130.604,11.419 C137.009,11.419 139.436,13.924 139.436,18.223 C139.436,20.968 137.885,23.074 135.139,24.109 L140.351,32.663 L140.351,32.663 z M143.077,32.663 L157.875,32.663 L157.875,28.608 L148.884,28.608 L157.678,14.922 L157.678,11.419 L143.277,11.419 L143.277,15.472 L151.867,15.472 L143.077,29.162 L143.077,32.663 L143.077,32.663 z M167.281,23.912 L167.281,28.685 L170.706,28.685 C173.055,28.685 174.088,27.849 174.088,26.219 C174.088,24.547 172.773,23.912 170.706,23.912 L167.281,23.912 L167.281,23.912 z M167.281,15.394 L167.281,20.01 L170.149,20.01 C171.622,20.01 172.892,19.335 172.892,17.626 C172.892,15.874 171.42,15.394 169.952,15.394 L167.281,15.394 L167.281,15.394 z M162.669,32.663 L162.669,11.419 L170.029,11.419 C173.731,11.419 177.791,12.254 177.791,16.87 C177.791,19.218 176.434,20.886 174.251,21.563 L174.251,21.602 C177.033,21.959 178.784,23.792 178.784,26.536 C178.784,30.954 174.565,32.663 170.787,32.663 L162.669,32.663 L162.669,32.663 z M184.984,32.663 L180.324,32.663 L187.886,11.419 L193.613,11.419 L201.171,32.663 L196.201,32.663 L194.728,28.328 L186.451,28.328 L184.984,32.663 L184.984,32.663 z M193.377,24.35 L191.507,18.699 C190.992,17.147 190.592,15.715 190.592,15.715 C190.592,15.715 190.193,17.147 189.677,18.739 L187.805,24.35 L193.377,24.35 L193.377,24.35 z M204.059,32.663 L208.357,32.663 L208.357,22.558 C208.357,20.65 208.238,18.699 208.238,18.699 C208.238,18.699 209.233,20.408 210.344,22.076 L217.544,32.663 L221.642,32.663 L221.642,11.419 L217.345,11.419 L217.345,21.049 C217.345,22.952 217.469,24.907 217.469,24.907 C217.469,24.907 216.471,23.196 215.358,21.521 L208.477,11.419 L204.059,11.419 L204.059,32.663 L204.059,32.663 z M226.75,11.419 L226.75,32.663 L231.366,32.663 L231.366,22.717 L238.17,32.663 L243.859,32.663 L236.064,21.285 L243.105,11.419 L237.771,11.419 L231.366,20.968 L231.366,11.419 L226.75,11.419 L226.75,11.419 z"
}), /*#__PURE__*/React__default.createElement("path", {
  id: "Bildmarke",
  d: "M289.998,30.987 C289.979,30.836 289.969,30.741 289.953,30.6 C289.929,30.39 289.895,30.164 289.873,30.006 C289.845,29.802 289.78,29.428 289.733,29.207 C289.73,29.189 289.713,29.177 289.695,29.177 L276.994,29.177 C276.994,29.177 266.659,29.176 265.277,29.175 C265.18,29.175 265.091,29.175 265.074,29.175 C262.067,29.119 259.373,28.139 257.088,26.508 C257.053,26.483 257.009,26.523 257.031,26.561 L264.777,39.98 C264.784,39.992 264.797,40 264.811,40 L287.657,40 C287.67,40 287.682,39.993 287.69,39.983 C288.594,38.731 289.255,37.338 289.658,35.836 C290.034,34.434 290.167,33 290.055,31.567 C290.04,31.373 290.022,31.18 289.998,30.987 z M288.34,1.097 C288.333,1.086 288.321,1.078 288.308,1.077 C287.969,1.049 287.629,1.036 287.289,1.036 L287.288,1.036 C285.098,1.036 283.023,1.605 281.116,2.682 C280.152,3.227 279.264,3.895 278.477,4.67 C278.382,4.763 278.289,4.857 278.198,4.953 C278.186,4.966 278.184,4.985 278.193,5 C278.636,5.769 288.818,23.424 290.326,26.052 C292.787,30.339 291.714,34.827 291.368,36.064 C291.355,36.109 291.417,36.135 291.441,36.095 L299.984,21.301 C299.992,21.289 299.992,21.273 299.984,21.261 L288.34,1.097 z M266.259,0.02 L254.276,20.776 C254.269,20.789 254.269,20.805 254.277,20.818 C254.416,21.035 255.64,22.932 256.473,23.765 C257.637,24.93 258.993,25.844 260.502,26.482 C261.311,26.825 262.058,27.08 262.913,27.246 C262.93,27.249 262.947,27.241 262.956,27.226 C263.415,26.432 274.878,6.586 274.912,6.527 C276.741,3.359 279.595,1.15 282.813,0.077 C282.856,0.063 282.846,-0 282.801,-0 L266.293,-0 C266.279,-0 266.266,0.007 266.259,0.02 z"
}))));

exports.LogoPresentation = LogoPresentation;
